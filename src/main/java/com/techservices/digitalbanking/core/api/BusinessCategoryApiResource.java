...
@Tag(
    ...
    description = "Business category and sector management endpoints")
public class BusinessCategoryApiResource {
  ...
  @Operation(
      ...
      description = "Retrieve business categories by type (PRIVATE_SECTOR or PUBLIC_SECTOR)")
  public ResponseEntity<BusinessCategoryResponse> getCategoriesByType(
      @PathVariable BusinessCategoryType categoryType,
      @Parameter(description = "Include category items in response")
          @RequestParam(defaultValue = "false")
          boolean includeItems) {

    log.info(
        "GET /api/v1/business-categories/type/{} - includeItems: {}", categoryType, includeItems);
    BusinessCategoryResponse categories =
        businessCategoryService.getCategoriesByType(categoryType, includeItems);
    return ResponseEntity.ok(categories);
  }

  @GetMapping("/private-sector")
  @Operation(
      summary = "Get private sector categories",
      description = "Retrieve all private sector categories")
  public ResponseEntity<BusinessCategoryResponse> getPrivateSectorCategories(
    ...
    BusinessCategoryResponse categories =
        businessCategoryService.getPrivateSectorCategories(includeItems);
    return ResponseEntity.ok(categories);
  }

  @GetMapping("/public-sector")
  @Operation(
      summary = "Get public sector categories",
      description = "Retrieve all public sector categories")
  public ResponseEntity<BusinessCategoryResponse> getPublicSectorCategories(
    ...
    BusinessCategoryResponse categories =
        businessCategoryService.getPublicSectorCategories(includeItems);
    return ResponseEntity.ok(categories);
  }

  @GetMapping("/{id}")
  @Operation(summary = "Get category by ID", description = "Retrieve a business category by its ID")
  public ResponseEntity<BusinessCategoryResponse> getCategoryById(
  ...
  @Operation(
      ...
      description = "Retrieve a business category by its code")
  public ResponseEntity<BusinessCategoryResponse> getCategoryByCode(
    ...
    BusinessCategoryResponse category =
        businessCategoryService.getCategoryByCode(code, includeItems);
    return ResponseEntity.ok(category);
  }
  ...
  @Operation(
      summary = "Get category item by code",
      description = "Retrieve a specific item by category code and item code")
  public ResponseEntity<BusinessCategoryItemResponse> getCategoryItemByCode(
      @PathVariable String categoryCode, @PathVariable String itemCode) {

    log.info("GET /api/v1/business-categories/{}/items/{}", categoryCode, itemCode);
    BusinessCategoryItemResponse item =
        businessCategoryService.getCategoryItemByCode(categoryCode, itemCode);
