...
@Slf4j
@Tag(name = "Business Categories Admin", description = "Admin endpoints for managing business categories")
public class BusinessCategoryAdminApiResource {
    ...
    public ResponseEntity<GenericApiResponse> activateCategory(@PathVariable Long categoryId) {
        ...
        return ResponseEntity.ok(new GenericApiResponse("Category activated successfully", "success", null));
    }
    ...
    public ResponseEntity<GenericApiResponse> deactivateCategory(@PathVariable Long categoryId) {
        ...
        businessCategoryRepository.save(category);
        return ResponseEntity.ok(new GenericApiResponse("Category deactivated successfully", "success", null));
    }

    @PutMapping("/items/{itemId}/activate")
    @Operation(summary = "Activate a category item", description = "Activate a business category item")
    public ResponseEntity<GenericApiResponse> activateCategoryItem(@PathVariable Long itemId) {
        ...
        BusinessCategoryItem item = businessCategoryItemRepository.findById(itemId)
                .orElseThrow(() -> new RuntimeException("Category item not found"));
        item.setIsActive(true);
        businessCategoryItemRepository.save(item);
        return ResponseEntity.ok(new GenericApiResponse("Category item activated successfully", "success", null));
    }

    @PutMapping("/items/{itemId}/deactivate")
    @Operation(summary = "Deactivate a category item", description = "Deactivate a business category item")
    public ResponseEntity<GenericApiResponse> deactivateCategoryItem(@PathVariable Long itemId) {
        ...
        businessCategoryItemRepository.save(item);
        return ResponseEntity.ok(new GenericApiResponse("Category item deactivated successfully", "success", null));
    }
}
