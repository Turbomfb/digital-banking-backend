...
public class IdentityVerificationService {
	...
	public IdentityVerificationResponse retrieveBvnData(String bvn) {
		...
		Optional<IdentityVerificationData> data = identityVerificationDataRepository.findByIdentifierAndType(bvn,
				IdentityVerificationDataType.BVN.name());
		if (data.isPresent()) {
	...
	public CustomerIdentityVerificationResponse verifyBvn(String bvn, Customer foundCustomer) {
		...
		return processVerificationResponse(identityVerificationResponse, foundCustomer,
				IdentityVerificationDataType.BVN);
	}
	...
	public IdentityVerificationResponse retrieveNinData(String nin) {
		...
		Optional<IdentityVerificationData> data = identityVerificationDataRepository.findByIdentifierAndType(nin,
				IdentityVerificationDataType.NIN.name());
		if (data.isPresent()) {
	...
	public CustomerIdentityVerificationResponse verifyNin(String nin, Customer foundCustomer) {
		...
		return processVerificationResponse(identityVerificationResponse, foundCustomer,
				IdentityVerificationDataType.NIN);
	}
	...
	public CustomerIdentityVerificationResponse verifyRcNumber(String rcNumber, Customer foundCustomer) {
		...
		IdentityVerificationResponse identityVerificationResponse = IdentityVerificationResponse
				.parse(businessDataResponse.getData());
		return processVerificationResponse(identityVerificationResponse, foundCustomer,
				IdentityVerificationDataType.RC_NUMBER);
	}
	...
	public CustomerIdentityVerificationResponse verifyTin(String tin, Customer foundCustomer) {
		...
		IdentityVerificationResponse identityVerificationResponse = IdentityVerificationResponse
				.parse(businessDataResponse.getData());
		return processVerificationResponse(identityVerificationResponse, foundCustomer,
				IdentityVerificationDataType.TIN);
	}
	...
	public IdentityVerificationResponse fetchIdentityVerificationResponse(String url, String identifier) {
		...
		try {
			...
			return apiService.callExternalApi(url, IdentityVerificationResponse.class, HttpMethod.POST, requestPayload,
					headers);
	...
	private ImageComparisonResponse compareImage(ImageComparisonRequest imageComparisonRequest) {
		...
		try {
			...
			HttpHeaders headers = getTokenHeader();
			return apiService.callExternalApi(url, ImageComparisonResponse.class, HttpMethod.POST,
					imageComparisonRequest, headers);
		} catch (PlatformServiceException e) {
			log.error(e.getDefaultUserMessage());
			throw new ValidationException("verification.failed", "Verification failed", e.getDefaultUserMessage());
		} catch (JsonProcessingException e) {
	...
	private CustomerIdentityVerificationResponse processVerificationResponse(IdentityVerificationResponse response,
			Customer foundCustomer, IdentityVerificationDataType dataType) {
  ...
  private static double calculateSimilarityPercentage(String str1, String str2) {
	...
	public void validateImageMismatch(String base64Image, String bvn, String nin) {
		...
		if (StringUtils.isNotBlank(base64Image)) {
			...
			ImageComparisonRequest imageComparisonRequest = ImageComparisonRequest.builder().isSubjectConsent(true)
					.image1(image2).image2(base64Image).build();
			...
			if (compareResponse.isSuccess()) {
				if (compareResponse.getData() == null || compareResponse.getData().getImageComparison() == null
					...
					throw new ValidationException("validation.error.imageMismatch",
							"Provided image does not match ID image");
				} else
					return;
			}
	...
	public BusinessDataResponse retrieveRcNumberData(String rcNumber) {
		...
		try {
			Optional<IdentityVerificationData> data = identityVerificationDataRepository
					.findByIdentifierAndType(rcNumber, IdentityVerificationDataType.RC_NUMBER.name());
			if (data.isPresent()) {
				log.info("RC Number data found in cache for: {} <==> {}", rcNumber, data.get());
				return convertToBusinessDataResponse(data.get());
			}

			String url = systemProperty.getYouverifyIntegrationUrl()
			...
			BusinessDataResponse response = apiService.callExternalApi(url, BusinessDataResponse.class, HttpMethod.POST,
					businessDataRequest, headers);
	...
	public BusinessDataResponse retrieveTinData(String tin) {
		...
		try {
			Optional<IdentityVerificationData> data = identityVerificationDataRepository.findByIdentifierAndType(tin,
					IdentityVerificationDataType.TIN.name());
			if (data.isPresent()) {
