...
public class BusinessCategoryService {
  ...
  public List<BusinessCategoryResponse> getAllCategories(boolean includeItems) {
    ...
    List<BusinessCategory> categories =
        businessCategoryRepository.findByIsActiveTrueOrderByDisplayOrderAsc();
  ...
  @Transactional(readOnly = true)
  public BusinessCategoryResponse getCategoriesByType(
      BusinessCategoryType categoryType, boolean includeItems) {

    log.info(
        "Fetching business categories by type: {}, includeItems: {}", categoryType, includeItems);
    return businessCategoryRepository
        .findByCategoryTypeAndIsActiveTrueOrderByDisplayOrderAsc(categoryType)
        .map(category -> BusinessCategoryResponse.from(category, includeItems))
        .orElseThrow(
            () ->
                new ValidationException(
                    "category.type.not.found", "Business category type not found"));
  }
        ...
        includeItems
            ? businessCategoryRepository
                ...
                .orElseThrow(
                    () ->
                        new ValidationException(
                            "category.not.found", "Business category not found"))
            : businessCategoryRepository
                .findById(id)
                .orElseThrow(
                    () ->
  ...
  public List<BusinessCategoryItemResponse> getCategoryItems(Long categoryId) {
    ...
    List<BusinessCategoryItem> items =
        businessCategoryItemRepository.findByCategoryIdAndIsActiveTrueOrderByDisplayOrderAsc(
            categoryId);
    return items.stream().map(BusinessCategoryItemResponse::from).collect(Collectors.toList());
  }
  ...
  public BusinessCategoryItemResponse getCategoryItemByCode(String categoryCode, String itemCode) {
    ...
    BusinessCategoryItem item =
        businessCategoryItemRepository
            .findByCategoryCodeAndItemCode(categoryCode, itemCode)
            .orElseThrow(
                () ->
                    new ValidationException(
                        "category.item.not.found", "Business category item not found"));
