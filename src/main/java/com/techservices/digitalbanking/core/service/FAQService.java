...
public class FAQService {
	...
	public BasePageResponse<FAQDto> getAllActiveFAQs() {
		...
		return BasePageResponse
				.instance(faqRepository.findByDelFlgFalse().stream().map(FAQDto::new).collect(Collectors.toList()));
	}

	@Transactional(readOnly = true)
	public BasePageResponse<FAQDto> getFAQsByProduct(ProductType product) {

		return BasePageResponse.instance(faqRepository.findByProductAndDelFlgFalse(product).stream().map(FAQDto::new)
	...
	public BasePageResponse<FAQDto> getWebEnabledFAQs() {
		...
		return BasePageResponse.instance(faqRepository.findByWebEnableTrueAndDelFlgFalse().stream().map(FAQDto::new)
				.collect(Collectors.toList()));
	}
	...
	public BasePageResponse<FAQDto> getMobileEnabledFAQs() {
		...
		return BasePageResponse.instance(faqRepository.findByMobileEnableTrueAndDelFlgFalse().stream().map(FAQDto::new)
				.collect(Collectors.toList()));
	}
	...
	public BasePageResponse<FAQDto> searchFAQs(ProductType product, Boolean webEnable, Boolean mobileEnable) {
		...
		return BasePageResponse.instance(faqRepository.findByFilters(product, webEnable, mobileEnable).stream()
	...
	public Optional<FAQDto> getFAQById(Long id) {
		...
		return faqRepository.findById(id).filter(faq -> !faq.getDelFlg()).map(FAQDto::new);
	}
	...
	public FAQDto updateFAQ(Long id, CreateFAQRequest request) {
		...
		FAQ faq = faqRepository.findById(id).orElseThrow(() -> new RuntimeException("FAQ not found with id: " + id));
