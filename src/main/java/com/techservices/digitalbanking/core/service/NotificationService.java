...
public class NotificationService {
  ...
  public NotificationResponse sendSms(String phoneNumber, String message) {
    ...
    try {
      ...
      return apiService.callExternalApi(
          url, NotificationResponse.class, HttpMethod.POST, null, null);
    } catch (Exception e) {
  ...
  private String buildSmsUrl(String phoneNumber, String message) {
    ...
    if (phoneNumber == null || !phoneNumber.startsWith("+")) {
      if (phoneNumber == null || phoneNumber.isEmpty()) {
        throw new ValidationException(
            "invalid.phone.number", "Phone number cannot be null or empty.");
      }
    ...
    return String.format(
        "%s?to=%s&from=%s&sms=%s&type=plain&channel=generic&api_key=%s",
        baseUrl, phoneNumber, senderId, message, apiKey);
  }
  ...
  public void notifyUser(Customer customer, String message, AlertType alertType) {
    ...
    Optional<AlertPreference> alertPreference =
        alertPreferenceService.getPreferencesForCustomerByAlertType(customer.getId(), alertType);
    if (alertPreference.isPresent()) {
      ...
      log.info(
          "No alert preferences set for customer {} and alert type {}",
          customer.getId(),
          alertType);
    }
  }
