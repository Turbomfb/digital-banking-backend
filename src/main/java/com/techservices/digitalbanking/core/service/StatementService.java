...
public class StatementService {
  ...
  public byte[] generatePdfStatement(StatementRequest request, Customer customer)
      throws IOException {
    ...
    try {
      ...
      StatementHeaderFooter headerFooter =
          new StatementHeaderFooter(
              bankConfigurationService.getBankName(), bankConfigurationService.getBankAddress());
  ...
  public byte[] generateExcelStatement(StatementRequest request, Customer customer)
      throws IOException {
    ...
    try (Workbook workbook = new XSSFWorkbook()) {
      ...
      rowNum =
          addExcelAccountInfo(
              statementSheet, accountData, request, rowNum, headerStyle, dataStyle, customer);

      // Add statement summary
      rowNum =
          addExcelStatementSummary(
              statementSheet,
              accountData,
              transactions,
              request,
      ...
      // Add transactions
      addExcelTransactionData(
          statementSheet, transactions, rowNum, dataStyle, currencyStyle, dateStyle);
  ...
  private void addBankHeader(Document document) throws DocumentException {
    ...
    try {
      ...
      // Statement title
      Paragraph title =
          new Paragraph("ACCOUNT STATEMENT", new Font(Font.FontFamily.HELVETICA, 16, Font.BOLD));
      ...
      log.warn(
          "Failed to load bank logo from URL: {}", bankConfigurationService.getBankLogoUrl(), e);
    }
  }

  private void addAccountInformation(
      Document document, AccountDto accountData, StatementRequest request, Customer customer)
  ...
  private void addStatementSummary(
      Document document,
    ...
    addSummaryRow(
        summaryTable,
        "Available Balance:",
        formatCurrency(accountData.getAccountBalance()),
        summaryFont);
  ...
  private void addTransactionTable(Document document, List<TransactionDto> transactions)
    ...
    for (TransactionDto transaction : transactions) {
      ...
      addTransactionCell(table, formatDate(LocalDate.from(transaction.getDate())), dataFont);
      addTransactionCell(
          table, transaction.getReference() != null ? transaction.getReference() : "", dataFont);
  ...
  private int addExcelAccountInfo(
      Sheet sheet,
      AccountDto accountData,
      StatementRequest request,
      int startRow,
  ...
  private int addExcelStatementSummary(
      Sheet sheet,
      AccountDto accountData,
      List<TransactionDto> transactions,
      StatementRequest request,
  ...
  private void addExcelTransactionData(
      Sheet sheet,
      List<TransactionDto> transactions,
      int startRow,
      CellStyle dataStyle,
  ...
  private void createSummarySheet(Workbook workbook, List<TransactionDto> transactions) {
    ...
    long creditCount =
        transactions.stream().filter(t -> "CREDIT".equals(t.getTransactionType())).count();
    row = summarySheet.createRow(rowNum++);
    ...
    long debitCount =
        transactions.stream().filter(t -> "DEBIT".equals(t.getTransactionType())).count();
    row = summarySheet.createRow(rowNum++);
    row.createCell(0).setCellValue("Debit Transactions:");
    row.createCell(1).setCellValue(debitCount);

    // Average transaction amount
    Optional<BigDecimal> avgAmountOptional =
  ...
  private static class StatementHeaderFooter extends PdfPageEventHelper {
    ...
    public void onEndPage(PdfWriter writer, Document document) {
      ...
      // Add page number
      Phrase pagePhrase =
          new Phrase("Page " + writer.getPageNumber(), new Font(Font.FontFamily.HELVETICA, 8));
      ColumnText.showTextAligned(
          cb, Element.ALIGN_RIGHT, pagePhrase, document.right(), document.bottom() - 10, 0);

      Phrase timestampPhrase =
          new Phrase(
              "Generated on: "
                  + LocalDateTime.now().format(DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm")),
              new Font(Font.FontFamily.HELVETICA, 8));
      ColumnText.showTextAligned(
          cb, Element.ALIGN_LEFT, timestampPhrase, document.left(), document.bottom() - 10, 0);
    }
  }
}
