...
public class StatementService {
	...
	public byte[] generatePdfStatement(StatementRequest request, Customer customer) throws IOException {
		...
		try {
			...
			StatementHeaderFooter headerFooter = new StatementHeaderFooter(bankConfigurationService.getBankName(),
					bankConfigurationService.getBankAddress());
	...
	public byte[] generateExcelStatement(StatementRequest request, Customer customer) throws IOException {
		...
		try (Workbook workbook = new XSSFWorkbook()) {
			...
			// Add statement summary
			rowNum = addExcelStatementSummary(statementSheet, accountData, transactions, request, rowNum, headerStyle,
					currencyStyle);
	...
	private List<TransactionDto> getFilteredTransactions(StatementRequest request) {
		...
		BasePageResponse<TransactionDto> transactionResult = walletAccountTransactionService
				.retrieveSavingsAccountTransactions(request.getCustomerId(), request.getStartDate().toString(),
						request.getEndDate().toString(), request.getLimit(), TransactionType.ALL);
	...
	private void addBankHeader(Document document) throws DocumentException {
		...
		try {
			...
			bankDetails.add(new Phrase("Phone: " + bankConfigurationService.getBankSupportPhone() + " | Email: "
					+ bankConfigurationService.getBankSupportEmail() + "\n", addressFont));

			document.add(bankDetails);

			// Statement title
			Paragraph title = new Paragraph("ACCOUNT STATEMENT", new Font(Font.FontFamily.HELVETICA, 16, Font.BOLD));
	...
	private void addAccountInformation(Document document, AccountDto accountData, StatementRequest request,
			Customer customer) throws DocumentException {
	...
	private void addStatementSummary(Document document, AccountDto accountData, List<TransactionDto> transactions,
	...
	private void addStatementFooter(Document document) throws DocumentException {
		...
		footer.add(new Phrase("For any queries, please contact us at " + bankConfigurationService.getBankSupportPhone()
				+ " or " + bankConfigurationService.getBankSupportEmail() + "\n", footerFont));
	...
	private int addExcelAccountInfo(Sheet sheet, AccountDto accountData, StatementRequest request, int startRow,
			CellStyle headerStyle, CellStyle dataStyle, Customer customer) {
	...
	private int addExcelStatementSummary(Sheet sheet, AccountDto accountData, List<TransactionDto> transactions,
			StatementRequest request, int startRow, CellStyle headerStyle, CellStyle currencyStyle) {
	...
	private int addExcelTransactionHeaders(Sheet sheet, int startRow, CellStyle headerStyle) {
		...
		String[] headers = {"Transaction Date", "Value Date", "Reference", "Description", "Transaction Type",
				"Debit Amount", "Credit Amount", "Running Balance", "Status"};
	...
	private void addExcelTransactionData(Sheet sheet, List<TransactionDto> transactions, int startRow,
	...
	private void createSummarySheet(Workbook workbook, List<TransactionDto> transactions) {
		...
		long creditCount = transactions.stream().filter(t -> "CREDIT".equals(t.getTransactionType())).count();
		row = summarySheet.createRow(rowNum++);
		row.createCell(0).setCellValue("Credit Transactions:");
		row.createCell(1).setCellValue(creditCount);

		// Debit transactions count
		long debitCount = transactions.stream().filter(t -> "DEBIT".equals(t.getTransactionType())).count();
		row = summarySheet.createRow(rowNum++);
		...
		Optional<BigDecimal> avgAmountOptional = transactions.isEmpty()
				? Optional.empty()
				: Optional.of(
						transactions.stream().map(TransactionDto::getAmount).reduce(BigDecimal.ZERO, BigDecimal::add)
	...
	private BigDecimal calculateTotalDebits(List<TransactionDto> transactions) {
		...
		return transactions.stream().filter(t -> "DEBIT".equals(t.getTransactionType())).map(TransactionDto::getAmount)
				.reduce(BigDecimal.ZERO, BigDecimal::add);
	}

	private BigDecimal calculateTotalCredits(List<TransactionDto> transactions) {

		return transactions.stream().filter(t -> "CREDIT".equals(t.getTransactionType())).map(TransactionDto::getAmount)
				.reduce(BigDecimal.ZERO, BigDecimal::add);
	}
	...
	private static class StatementHeaderFooter extends PdfPageEventHelper {
		...
		public void onEndPage(PdfWriter writer, Document document) {
			...
			// Add page number
			Phrase pagePhrase = new Phrase("Page " + writer.getPageNumber(), new Font(Font.FontFamily.HELVETICA, 8));
			ColumnText.showTextAligned(cb, Element.ALIGN_RIGHT, pagePhrase, document.right(), document.bottom() - 10,
					0);

			Phrase timestampPhrase = new Phrase(
					"Generated on: " + LocalDateTime.now().format(DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm")),
					new Font(Font.FontFamily.HELVETICA, 8));
			ColumnText.showTextAligned(cb, Element.ALIGN_LEFT, timestampPhrase, document.left(), document.bottom() - 10,
					0);
		}
	}
}
