...
public class AlertPreferenceService {
	...
	@Transactional
	public AlertPreference updatePreference(Long customerId, AlertType alertType, boolean viaSms, boolean viaEmail,
			boolean viaPush) {

		AlertPreference preference = alertPreferenceRepository.findByCustomerIdAndAlertType(customerId, alertType)
				.orElseGet(() -> {
	...
	public AlertPreference enableChannel(Long customerId, AlertType alertType, String channel) {
		...
		AlertPreference preference = preferenceOptional.orElseGet(() -> {
			AlertPreference newPref = new AlertPreference();
			newPref.setCustomerId(customerId);
			newPref.setAlertType(alertType);
			return newPref;
