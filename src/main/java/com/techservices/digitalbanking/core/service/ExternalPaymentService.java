...
public class ExternalPaymentService {
	...
	public ExternalPaymentTransactionOtpGenerationResponse generateOtp(SavingsAccountTransactionRequest request) {
		...
		ExternalPaymentTransactionOtpGenerationResponse response = this.processRequest(url, request,
				ExternalPaymentTransactionOtpGenerationResponse.class, null);
	...
	public ExternalPaymentTransactionOtpVerificationResponse verifyOtp(SavingsAccountTransactionRequest request) {
		...
		ExternalPaymentTransactionOtpVerificationResponse response = this.processRequest(url, request,
				ExternalPaymentTransactionOtpVerificationResponse.class, null);
		if (!response.isSuccessful()) {
			throw new ValidationException("external.payment.service.error", response.getMessage());
		}
	...
	private <T> T processRequest(String url, Object request, Class<T> responseType, HttpHeaders headers) {
		...
		try {
			...
			throw new ValidationException("external.payment.service.error",
					"Error processing external payment transaction", e.getMessage());
		}
	}
	...
	private String generateAccessToken() {
		...
		try {
			ExternalPaymentServiceAuthenticationResponse response = apiService.callExternalApi(url,
					ExternalPaymentServiceAuthenticationResponse.class, HttpMethod.POST, params, null);
			if (!response.isSuccessful()) {
				throw new ValidationException("external.payment.service.error", response.getMessage());
			}
			...
			throw new ValidationException("external.payment.service.error",
					"Error generating access token: " + e.getMessage());
		}
	}

	public WalletPaymentOrderResponse initiateOrder(WalletPaymentOrderRequest dto, Customer customer, String reference)
			throws Exception {
		...
		if (dto == null) {
			...
			throw new ValidationException("external.payment.service.error", "Payment request cannot be null");
		}
		...
		if (reference == null || reference.trim().isEmpty()) {
			...
			throw new ValidationException("external.payment.service.error",
					"Payment reference cannot be null or empty");
		}
		...
		if (encodedKey == null || encodedKey.trim().isEmpty()) {
			...
			throw new ValidationException("external.payment.service.error", "PayInvert public key not configured");
		}
		...
		if (encryptedParts.length != 3) {
			log.error("❌ Encrypted data format invalid. Expected 3 parts, got: {}", encryptedParts.length);
			throw new ValidationException("external.payment.service.error", "Encryption resulted in invalid format");
		}
	...
	private Map<String, Object> buildOrderPayloadWithValidation(WalletPaymentOrderRequest dto, String reference,
		...
		if (mobile == null || mobile.trim().isEmpty()) {
			...
			throw new ValidationException("external.payment.service.error", "Customer mobile is required");
		}
		...
		if (reference == null || reference.trim().isEmpty()) {
			...
			throw new ValidationException("external.payment.service.error", "Order reference is required");
		}
		...
		log.info("✅ Order object built - Amount: {}, Reference: [{}], Currency: {}", dto.getAmount(), reference,
				dto.getCurrency());

		String redirectUrl = systemProperty.getClientPaymentRedirectUrl();
		if (redirectUrl == null || redirectUrl.trim().isEmpty()) {
			log.error("❌ Redirect URL is null/empty");
			throw new ValidationException("external.payment.service.error", "Redirect URL not configured");
		}
		...
		if (!payload.containsKey("order")) {
			...
			throw new ValidationException("external.payment.service.error", "Payload missing order after building");
		}
		...
		if (!orderFromPayload.containsKey("reference")) {
			...
			throw new ValidationException("external.payment.service.error", "Order missing reference after building");
		}

		Object referenceFromPayload = orderFromPayload.get("reference");
		if (!reference.equals(referenceFromPayload)) {
			log.error("❌ Reference mismatch - Expected: [{}], Got: [{}]", reference, referenceFromPayload);
			throw new ValidationException("external.payment.service.error", "Reference mismatch in payload");
		}
	...
	private void validatePayloadStructure(Map<String, Object> payload) {
		...
		for (String key : requiredKeys) {
			...
			if (payload.get(key) == null) {
				...
				throw new ValidationException("external.payment.service.error", "Payload field is null: " + key);
			}
		}
		...
		for (String key : orderRequiredKeys) {
			if (!order.containsKey(key)) {
				...
				throw new ValidationException("external.payment.service.error", "Order missing required field: " + key);
			}

			if (order.get(key) == null) {
				log.error("❌ Order key [{}] is null", key);
				throw new ValidationException("external.payment.service.error", "Order field is null: " + key);
			}
		}
