...
public class ExternalPaymentService {
    ...
    public WalletPaymentOrderResponse initiateOrder(WalletPaymentOrderRequest dto, Customer customer, String reference) throws Exception {
        ...
        if (reference == null || reference.trim().isEmpty()) {
            ...
            throw new ValidationException("external.payment.service.error", "Payment reference cannot be null or empty");
        }
        ...
        if (!jsonPayload.contains(reference)) {
            ...
            throw new ValidationException("external.payment.service.error", "Payload missing reference after JSON conversion");
        }

        if (!jsonPayload.contains("\"order\"") && !jsonPayload.contains("\"Order\"")) {
            log.error("❌ JSON payload does not contain 'order' field");
            throw new ValidationException("external.payment.service.error", "Payload missing order field after JSON conversion");
        }
    ...
    private Map<String, Object> buildOrderPayloadWithValidation(WalletPaymentOrderRequest dto, String reference, Customer customerData) {
        ...
        log.info("✅ Order object built - Amount: {}, Reference: [{}], Currency: {}",
                dto.getAmount(), reference, dto.getCurrency());
    ...
    private void validatePayloadStructure(Map<String, Object> payload) {
        ...
        for (String key : requiredKeys) {
            if (!payload.containsKey(key)) {
                ...
                throw new ValidationException("external.payment.service.error", "Payload missing required field: " + key);
            }
