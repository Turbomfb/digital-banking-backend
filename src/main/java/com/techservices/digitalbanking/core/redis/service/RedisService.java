	...
	public OtpDto generateOtpRequest(Object request, OtpType otpType, NotificationRequestDto notificationRequestDto,
		...
		if (notificationRequestDto != null) {
			String message = switch (otpType) {
				...
				case TRANSFER ->
					String.format("You have initiated a transfer of %s. Your one-time password (OTP) is: %s", amount,
							otpDto.getOtp());
				case ADD_BENEFICIARY -> String.format(
						"You are adding a new beneficiary. Your OTP to complete this action is: %s", otpDto.getOtp());
				case DELETE_BENEFICIARY ->
					String.format("You are about to delete a beneficiary. Your OTP to confirm this action is: %s",
							otpDto.getOtp());
				case UPDATE_BENEFICIARY ->
					String.format("You are about to update a beneficiary. Your OTP to confirm this action is: %s",
							otpDto.getOtp());
			};
