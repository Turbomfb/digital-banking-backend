...
public class FineractPageResponse<E> implements Serializable {
  ...
  public static <E> FineractPageResponse<E> create(
      FineractPageTransactionResponse<E> savingsAccountTransactions) {
    ...
    response.setPageItems(
        savingsAccountTransactions.getContent().stream()
            .sorted(
                (a, b) -> {
                  if (a instanceof SavingsAccountTransactionData
                      && b instanceof SavingsAccountTransactionData) {
                    return ((SavingsAccountTransactionData) a)
                        .getId()
                        .compareTo(((SavingsAccountTransactionData) b).getId());
                  }
                  return 0;
                })
            .toList());
