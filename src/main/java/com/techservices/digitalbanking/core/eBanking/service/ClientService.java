...
public class ClientService {
    ...
    public GetClientsClientIdResponse searchClients(String nin, String bvn, String emailAddress, String phoneNumber, UserType userType) {
        return customerApiClient.retrieveAll(null, 5, nin, bvn, emailAddress, phoneNumber)
                .getPageItems()
                .stream()
    ...
    public AccountDto getFlexAccountByCustomer(Customer customer) {
        try {
            return getCustomerActualAccount(this.getAllFlexAccountByExternalId(customer.getExternalId()), customer.getFlexAccountId());
        } catch (Exception e) {
    ...
    public AccountDto getCustomerActualAccount(List<AccountDto> savingsAccounts, String accountNumber) {
        return savingsAccounts.stream().filter(account -> account.getAccountNumber().equals(accountNumber)).findFirst().orElseThrow(
                () -> new PlatformServiceException("account.not.found", "Account with account number " + accountNumber + " not found")
        );
    }
