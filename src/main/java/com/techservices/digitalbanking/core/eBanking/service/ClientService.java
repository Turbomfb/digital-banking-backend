...
public class ClientService {
	...
	public PutClientsClientIdResponse updateCustomer(CustomerUpdateRequest customerUpdateRequest, String clientId,
			UserType userType) {
	...
	public GetClientsClientIdResponse searchClients(String nin, String bvn, String emailAddress, String phoneNumber,
		...
		return customerApiClient.retrieveAll(null, 5, nin, bvn, emailAddress, phoneNumber).getPageItems().stream()
				.filter(client -> client.getLegalForm().getId() == userType.getId()).findAny().orElse(null);
	}
	...
	public AccountDto getCustomerWalletAccount(Customer customer) {
		...
		return getCustomerActualAccount(this.getAllWalletAccountByExternalId(customer.getExternalId()),
	...
	public AccountDto getFlexAccountByCustomer(Customer customer) {
		...
		try {
			return getCustomerActualAccount(this.getAllFlexAccountByExternalId(customer.getExternalId()),
					customer.getFlexAccountId());
		} catch (Exception e) {
			log.error("Error retrieving flex account for customer {}: {}", customer.getId(), e.getMessage());
	...
	public AccountDto getCustomerActualAccount(List<AccountDto> savingsAccounts, String accountNumber) {

		return savingsAccounts.stream().filter(account -> account.getAccountNumber().equals(accountNumber)).findFirst()
				.orElseThrow(() -> new PlatformServiceException("account.not.found",
