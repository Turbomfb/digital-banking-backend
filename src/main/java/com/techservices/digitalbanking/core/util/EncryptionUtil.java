...
public class EncryptionUtil {
  ...
  public static String encryptAesPublicKeyXml(String data, String encodedXmlKey) throws Exception {
    ...
    log.debug(
        "🔍 Final XML key preview: {}",
        xmlKey.substring(0, Math.min(200, xmlKey.length())) + "...");
    ...
    Document doc =
        factory
            .newDocumentBuilder()
            .parse(new ByteArrayInputStream(xmlKey.getBytes(StandardCharsets.UTF_8)));
    ...
    log.debug(
        "🔢 Modulus length: {}, Exponent length: {}", modulusB64.length(), exponentB64.length());
  ...
  private static ParsedKey parseXmlKey(String encodedKey) {
    ...
    try {
      ...
      String decodedContent =
          new String(Base64.getDecoder().decode(base64Content), StandardCharsets.UTF_8);

      log.debug(
          "📦 Decoded content: {}",
          decodedContent.substring(0, Math.min(100, decodedContent.length())) + "...");
      ...
      log.debug(
          "📦 Final XML content to parse: {}",
          xmlKey.substring(0, Math.min(100, xmlKey.length())) + "...");
