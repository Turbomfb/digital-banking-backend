	...
	public void handleError(ClientHttpResponse response) {
		...
		try {
			...
			if (isValidJson(errorBody)) {
				...
				throw new PlatformServiceException("error.from.external.service",
						"We're unable to process your request at this time. Please try again later.", errorBody);
			}
		} catch (IOException e) {
			log.error("Error processing external payment transaction {}", e.toString());
			throw new PlatformServiceException("error.from.external.service",
					"We're unable to process your request at this time. Please try again later.", e.getMessage());
		}
	}
