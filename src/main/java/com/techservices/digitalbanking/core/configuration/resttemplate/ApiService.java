...
public class ApiService {
  ...
  public <T> T callExternalApi(
    ...
    log.info(
        "Calling external API with details: URL={}, Method={}, Payload={}, Headers={}",
        url,
        method,
        requestPayload,
        headers);
    ...
    if (responseEntity.getStatusCode().is2xxSuccessful()) {
      log.info(
          "External API call successful: {} <==>", responseEntity.getStatusCode(), responseEntity);
    } else if (responseEntity.getStatusCode().is4xxClientError()) {
      log.error("External API call failed with client error: {}", responseEntity.getStatusCode());
      throw new PlatformServiceException(
          "Client error during external API call: " + responseEntity.getBody());
    } else if (responseEntity.getStatusCode().is5xxServerError()) {
      log.error("External API call failed with server error: {}", responseEntity.getStatusCode());
      throw new PlatformServiceException(
          "Server error during external API call: " + responseEntity.getBody());
    } else if (responseEntity.getStatusCode().is5xxServerError()) {
  ...
  public <T> T callExternalApi(
    ...
    log.info(
        "Calling external API with details: URL={}, Method={}, Payload={}",
        url,
        method,
        requestPayload);
