	...
	public Exception decode(String methodKey, Response response) {
		...
		if (response.body() == null) {
			return switch (response.status()) {
				case HTTP_BAD_REQUEST -> new AbstractPlatformDomainRuleException("error.msg.cba",
						"Bad request occurred, response body is null", List.of("No additional details provided"));
				case HTTP_NOT_FOUND -> new AbstractPlatformDomainRuleException("error.msg.cba",
						"Resource not found, response body is null", List.of("No additional details provided"));
				case HTTP_FORBIDDEN -> new UnAuthorizedUserException("error.msg.cba",
						"Access forbidden, response body is null", List.of("No additional details provided"));
				case HTTP_UNAUTHORIZED -> new UnAuthenticatedUserException("error.msg.cba",
						"Unauthorized access, response body is null", List.of("No additional details provided"));
				default -> new AbstractPlatformDomainRuleException("error.msg.cba",
						"Service error occurred, response body is null", List.of("No additional details provided"));
			};
		}
		...
		try (InputStream bodyIs = response.body().asInputStream()) {
			...
			return switch (response.status()) {
				...
				case HTTP_NOT_FOUND -> new AbstractPlatformDomainRuleException(errorCode, errorMessage, errorDetails);
			...
			return new RuntimeException("We're unable to process your request at this time. Please try again later.",
					e);
		}
	}
}
