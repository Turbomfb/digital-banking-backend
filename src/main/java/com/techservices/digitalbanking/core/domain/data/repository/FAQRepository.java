...
public interface FAQRepository extends JpaRepository<FAQ, Long> {
    ...
    @Query("SELECT f FROM FAQ f WHERE f.delFlg = false AND " +
            "(:product IS NULL OR f.product = :product) AND " +
            "(:webEnable IS NULL OR f.webEnable = :webEnable) AND " +
            "(:mobileEnable IS NULL OR f.mobileEnable = :mobileEnable)")
    List<FAQ> findByFilters(@Param("product") ProductType product,
                            @Param("webEnable") Boolean webEnable,
