...
public interface IdentityVerificationDataRepository
    extends JpaRepository<IdentityVerificationData, Long> {

  @Query(
      "SELECT ivd FROM IdentityVerificationData ivd WHERE LOWER(ivd.identifier) = LOWER(:identifier) and LOWER(ivd.type) = LOWER(:type)")
