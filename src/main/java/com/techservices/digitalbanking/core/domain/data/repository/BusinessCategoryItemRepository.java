...
public interface BusinessCategoryItemRepository extends JpaRepository<BusinessCategoryItem, Long> {
  ...
  @Query(
      "SELECT i FROM BusinessCategoryItem i WHERE i.category.code = :categoryCode AND i.code = :itemCode")
  Optional<BusinessCategoryItem> findByCategoryCodeAndItemCode(
      @Param("categoryCode") String categoryCode, @Param("itemCode") String itemCode);
