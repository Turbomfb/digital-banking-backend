...
@Entity
@Table(
    name = "name_enquiry_cache",
    uniqueConstraints = @UniqueConstraint(columnNames = {"account_number", "bank_code"}))
@Setter
@Getter
@ToString
public class NameEnquiryCache {
  ...
  public static NameEnquiryCache parse(
      NameEnquiryResponse response, String accountNumber, String bankCode) {
    ...
    if (response.getData() != null && response.getData().getBankDetails() != null) {
      NameEnquiryResponse.NameEnquiryResponseData.BankDetail bankDetail =
          response.getData().getBankDetails();
