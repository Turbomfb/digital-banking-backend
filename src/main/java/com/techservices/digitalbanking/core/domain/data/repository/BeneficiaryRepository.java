...
public interface BeneficiaryRepository extends JpaRepository<Beneficiary, Long> {
  ...
  Optional<Beneficiary> findByCustomerIdAndAccountNumberAndBankCode(
      Long customerId, String accountNumber, String bankCode);

  boolean existsByCustomerIdAndAccountNumberAndBankCode(
      Long customerId, String accountNumber, String bankCode);
  ...
  @Query(
      "SELECT b FROM Beneficiary b WHERE b.customerId = :customerId AND b.isActive = true ORDER BY b.usageCount DESC, b.lastUsedAt DESC")
  List<Beneficiary> findFrequentBeneficiaries(@Param("customerId") Long customerId);

  @Query(
      "SELECT COUNT(b) FROM Beneficiary b WHERE b.customerId = :customerId AND b.isActive = true")
  long countActiveByCustomerId(@Param("customerId") Long customerId);

  @Query(
      "SELECT b FROM Beneficiary b WHERE b.customerId = :customerId AND b.isActive = true "
  ...
  List<Beneficiary> searchBeneficiaries(
      @Param("customerId") Long customerId, @Param("searchTerm") String searchTerm);

  @Query(
      "SELECT b FROM Beneficiary b WHERE b.customerId = :customerId AND b.isActive = true "
          + "AND b.createdAt BETWEEN :startDate AND :endDate "
          + "ORDER BY b.createdAt DESC")
  List<Beneficiary> findByCustomerIdAndDateRange(
