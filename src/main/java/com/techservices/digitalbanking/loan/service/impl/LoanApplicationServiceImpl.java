...
public class LoanApplicationServiceImpl implements LoanApplicationService {
	...
	public BasePageResponse<LoanDto> retrieveAllLoans(Long limit, String accountNo, Long customerId) {
		...
		FilterDto filterDto = new FilterDto().emailAddress(customer.getEmailAddress())
				.customerType(customer.getUserType());
		return BasePageResponse.instance(loanService.retrieveAllCustomerLoans(filterDto).stream().filter(
				loan -> loan.getStatus() != null && (loan.isActive() || loan.isPending() || loan.isLiquidated()))
				.toList());
	}
	...
	public GenericApiResponse repayLoan(Long loanId, @Valid LoanRepaymentRequest loanRepaymentRequest,
		...
		accountTransactionService.handleWithdrawal(customer.getAccountId(), loanRepaymentRequest.getTransactionAmount(),
				null, "Loan Repayment");
	...
	@Override
	public LoanApplicationResponse processNewLoanApplication(Long customerId,
	...
	public LoanDashboardResponse retrieveCustomerLoanDashboard(Long customerId) {
		...
		return LoanDashboardResponse.builder().activeLoanBalance(getActiveLoanBalance(customerLoans))
				.totalExpectedRepayment(getTotalExpectedRepayment(customerLoans))
				.totalRepaid(getTotalRepayment(customerLoans)).activeLoanCount(getActiveLoanCount(customerLoans))
				.pendingLoanCount(getPendingLoanCount(customerLoans))
				.liquidatedLoanCount(getLiquidatedLoanCount(customerLoans)).build();
	}
	...
	public BigDecimal getActiveLoanBalance(List<LoanDto> customerLoans) {
		...
		return customerLoans.stream().filter(LoanDto::isActive)
	...
	public Long getPendingLoanCount(List<LoanDto> customerLoans) {
		...
		return customerLoans.stream().filter(loan -> loan.getStatus() != null && loan.isPending()).count();
	}

	public Long getLiquidatedLoanCount(List<LoanDto> customerLoans) {

		return customerLoans.stream().filter(loan -> loan.getStatus() != null && loan.isLiquidated()).count();
	}
}
