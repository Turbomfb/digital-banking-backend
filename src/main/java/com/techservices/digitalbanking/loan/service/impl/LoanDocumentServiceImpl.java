...
public class LoanDocumentServiceImpl implements LoanDocumentService {
  ...
  public GenericApiResponse uploadDocument(
      Long loanId, LoanDocumentUploadRequest request, Long customerId) {
  ...
  @Override
  public LoanDocumentBatchUploadResponse uploadDocuments(
      Long loanId, List<LoanDocumentUploadRequest> requests, Long customerId) {
    ...
    for (LoanDocumentUploadRequest request : requests) {
      try {
        ...
        LoanDocumentBatchUploadResponse.UploadError error =
            new LoanDocumentBatchUploadResponse.UploadError();
        error.setErrorMessage(e.getMessage());
        error.setDocumentName(request.getName());
        error.setFileName(request.getFile().getOriginalFilename());
        batchUploadResponse.addError(
            error.getDocumentName(), error.getFileName(), error.getErrorMessage(), "UPLOAD_ERROR");
