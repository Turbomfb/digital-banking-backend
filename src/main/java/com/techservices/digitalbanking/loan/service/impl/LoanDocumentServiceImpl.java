...
public class LoanDocumentServiceImpl implements LoanDocumentService {
	...
	@Override
	public LoanDocumentBatchUploadResponse uploadDocuments(Long loanId, List<LoanDocumentUploadRequest> requests,
			Long customerId) {
		...
		for (LoanDocumentUploadRequest request : requests) {
			try {
				...
				batchUploadResponse.addError(error.getDocumentName(), error.getFileName(), error.getErrorMessage(),
						"UPLOAD_ERROR");
