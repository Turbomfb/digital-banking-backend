...
public class LoanDocumentApiResource {
  ...
  public ResponseEntity<GenericApiResponse> uploadDocument(
    ...
    return ResponseEntity.ok(
        new GenericApiResponse("Document uploaded successfully", "success", response));
  }
  ...
  public ResponseEntity<GenericApiResponse> uploadDocuments(
      @Parameter(description = "Loan ID", required = true) @PathVariable Long loanId,
      @Parameter(description = "Array of document files", required = true) @RequestParam("files")
          List<MultipartFile> files,
      @Parameter(
              description = "Array of document names (must match files array length)",
              required = true)
          @RequestParam("names")
          List<String> names,
      @Parameter(description = "Array of document descriptions")
          @RequestParam(value = "descriptions", required = false)
          List<String> descriptions) {

    log.info(
        "POST /api/v1/loans/{}/documents/batch - Uploading {} documents", loanId, files.size());

    if (files.size() != names.size()) {
      return ResponseEntity.badRequest()
          .body(
              new GenericApiResponse(
                  "Files and names arrays must have the same length", "error", null));
    }
    ...
    Long customerId = springSecurityAuditorAware.getAuthenticatedUser().getUserId();
    LoanDocumentBatchUploadResponse response =
        loanDocumentService.uploadDocuments(loanId, requests, customerId);
    ...
    return ResponseEntity.ok(
        new GenericApiResponse(
            message, response.getFailedUploads() > 0 ? "partial_success" : "success", response));
  }
}
