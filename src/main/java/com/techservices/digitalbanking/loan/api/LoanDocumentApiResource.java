...
public class LoanDocumentApiResource {
	...
	@Operation(summary = "Upload single loan document", description = "Upload a single document for a loan application")
	public ResponseEntity<GenericApiResponse> uploadDocument(
			...
			@Parameter(description = "Document file", required = true) @RequestPart("file") MultipartFile file,
			@Parameter(description = "Document name", required = true) @RequestPart("name") String name,
			@Parameter(description = "Document description") @RequestPart(value = "description", required = false) String description) {
		...
		return ResponseEntity.ok(new GenericApiResponse("Document uploaded successfully", "success", response));
	}

	@PostMapping(value = "/batch", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
	@Operation(summary = "Upload multiple loan documents", description = "Upload multiple documents for a loan application in a single request")
	public ResponseEntity<GenericApiResponse> uploadDocuments(
			@Parameter(description = "Loan ID", required = true) @PathVariable Long loanId,
			@Parameter(description = "Array of document files", required = true) @RequestParam("files") List<MultipartFile> files,
			@Parameter(description = "Array of document names (must match files array length)", required = true) @RequestParam("names") List<String> names,
			@Parameter(description = "Array of document descriptions") @RequestParam(value = "descriptions", required = false) List<String> descriptions) {

		log.info("POST /api/v1/loans/{}/documents/batch - Uploading {} documents", loanId, files.size());

		if (files.size() != names.size()) {
			return ResponseEntity.badRequest()
					.body(new GenericApiResponse("Files and names arrays must have the same length", "error", null));
		}
		...
		return ResponseEntity.ok(new GenericApiResponse(message,
				response.getFailedUploads() > 0 ? "partial_success" : "success", response));
	}
}
