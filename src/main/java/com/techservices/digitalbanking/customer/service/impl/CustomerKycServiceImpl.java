...
public class CustomerKycServiceImpl implements CustomerKycService {
	...
	public BasePageResponse<KycTierDto> retrieveAllKycTier() {
		...
		return BasePageResponse.instance(kycTierRepository.findAll().stream().map(KycTierDto::parse).toList());
	}

	private BaseAppResponse handleOtpGeneration(CustomerKycRequest customerKycRequest, Customer foundCustomer,
			String command) {

		IdentityVerificationResponse verificationResponse = validateKycParameters(customerKycRequest, foundCustomer,
				command);
	...
	private GenericApiResponse processKycOtpGeneration(CustomerKycRequest customerKycRequest,
			IdentityVerificationResponse verificationResponse, UserType userType) {

		NotificationRequestDto notificationRequestDto = new NotificationRequestDto(verificationResponse);
		OtpDto otpDto = redisService.generateOtpRequest(customerKycRequest, OtpType.KYC_UPGRADE, notificationRequestDto,
				null);
	...
	private void validateOtpGenerationData(IdentityVerificationResponse verificationResponse,
			IdentityVerificationDataType dataType, Customer foundCustomer) {
		...
		if (verificationResponse.getData() != null) {
			...
			if ("EXTERNAL".equalsIgnoreCase(verificationResponse.getDataSource())) {
				...
				boolean isValidIndividualData = StringUtils.isNoneBlank(data.getFirstName(), data.getLastName(),
						data.getMobile());
				...
				if ((dataType.isIndividual() && !isValidIndividualData)
					...
					throw new ValidationException("validation.error.exists",
							"We're unable to complete your tier upgrade because the information provided does not match our records. "
									+ "Please confirm that your " + dataType
	...
	private CustomerDtoResponse handleOtpVerification(CustomerKycRequest customerKycRequest, Long customerId,
			Customer foundCustomer, String command) {
		...
		if (customerKycRequest == null) {
			throw new ValidationException("otp.expired",
					"Verification has expired or does not exist. Please initiate a new one.");
		}
		...
		processCustomerActivationOrUpdate(foundCustomer, customerKycRequest, customerId, customerKycTier,
	...
	private OtpDto retrieveAndValidateOtp(CustomerKycRequest customerKycRequest, String uniqueId, String otp) {
		...
		if (StringUtils.isNotBlank(customerKycRequest.getBase64Image())) {
			...
			identityVerificationService.validateImageMismatch(customerKycRequest.getBase64Image(),
					originalRequest.getBvn(), originalRequest.getNin());
	...
	private void processCustomerActivationOrUpdate(Customer foundCustomer, CustomerKycRequest customerKycRequest,
			Long customerId, CustomerKycTier customerKycTier, IdentityVerificationResponse verificationResponse) {
	...
	private void updateCustomerAddressIfNeeded(Customer foundCustomer,
	...
	private IdentityVerificationResponse validateKycParameters(CustomerKycRequest customerKycRequest,
			Customer foundCustomer, String command) {
	...
	private void validateBasicKycRequirements(CustomerKycRequest customerKycRequest, Customer foundCustomer) {
		...
		boolean includesNoKycParameter = StringUtils.isAllBlank(customerKycRequest.getBvn(),
				customerKycRequest.getNin(), customerKycRequest.getRcNumber(), customerKycRequest.getTin());
	...
	private IdentityVerificationResponse handleBvnNinValidation(CustomerKycRequest customerKycRequest,
			Customer foundCustomer, String command) {
		...
		if (GENERATE_OTP_COMMAND.equalsIgnoreCase(command)) {
			IdentityVerificationResponse ninVerificationResponse = identityVerificationService
					.retrieveNinData(customerKycRequest.getNin());
			validateOtpGenerationData(ninVerificationResponse, NIN, foundCustomer);
			IdentityVerificationResponse bvnVerificationResponse = identityVerificationService
					.retrieveBvnData(customerKycRequest.getBvn());
	...
	private void validateExistingBvnNin(Customer foundCustomer) {
		...
		if (StringUtils.isNotBlank(foundCustomer.getNin()) && StringUtils.isNotBlank(foundCustomer.getBvn())) {
			throw new ValidationException("bvn.and.nin.validation.exists",
					"Both BVN and NIN have already been provided.");
		}
	...
	private void validateBvnUniqueness(CustomerKycRequest customerKycRequest, Customer foundCustomer) {
		...
		customerRepository.findByBvnAndUserType(customerKycRequest.getBvn(), foundCustomer.getUserType())
				.ifPresent(existedCustomer -> {
					log.error("BVN {} already exists for customer id: {}", customerKycRequest.getBvn(),
	...
	private void validateNinUniqueness(CustomerKycRequest customerKycRequest, Customer foundCustomer) {
		...
		customerRepository.findByNinAndUserType(customerKycRequest.getNin(), foundCustomer.getUserType())
				.ifPresent(existingCustomer -> {
					log.error("NIN {} already exists for customer id: {}", customerKycRequest.getNin(),
							existingCustomer.getId());
					throw new ValidationException("nin.already.exists", "You have entered an invalid NIN.");
				});
	}

	private IdentityVerificationResponse processNinBvnVerification(CustomerKycRequest customerKycRequest,
			Customer foundCustomer) {

		CustomerIdentityVerificationResponse customerIdentityVerificationResponse = identityVerificationService
				.verifyNin(customerKycRequest.getNin(), foundCustomer);
		...
		customerIdentityVerificationResponse = identityVerificationService.verifyBvn(customerKycRequest.getBvn(),
				foundCustomer);
		...
		if (bvnData.getData().getAddress() == null) {
			IdentityVerificationResponse ninData = identityVerificationService
					.retrieveNinData(customerKycRequest.getNin());
	...
	private IdentityVerificationResponse handleRcNumberValidation(CustomerKycRequest customerKycRequest,
			Customer foundCustomer, String command) {

		log.info("Validating RC Number: {}", customerKycRequest.getRcNumber());
		BusinessDataResponse businessData = identityVerificationService
				.retrieveRcNumberData(customerKycRequest.getRcNumber());
		IdentityVerificationResponse rcNumberIdentityVerificationResponse = IdentityVerificationResponse
				.parse(businessData.getData(), EXTERNAL);
		if (GENERATE_OTP_COMMAND.equalsIgnoreCase(command)) {
		...
		CustomerIdentityVerificationResponse customerIdentityVerificationResponse = identityVerificationService
				.verifyRcNumber(customerKycRequest.getRcNumber(), foundCustomer);
	...
	private IdentityVerificationResponse handleTinValidation(CustomerKycRequest customerKycRequest,
			Customer foundCustomer, String command) {

		BusinessDataResponse businessData = identityVerificationService.retrieveTinData(customerKycRequest.getTin());
		IdentityVerificationResponse tinIdentityVerificationResponse = IdentityVerificationResponse
				.parse(businessData.getData(), EXTERNAL);
		if (GENERATE_OTP_COMMAND.equalsIgnoreCase(command)) {
		...
		CustomerIdentityVerificationResponse customerIdentityVerificationResponse = identityVerificationService
				.verifyTin(customerKycRequest.getTin(), foundCustomer);
	...
	private static void finalizeValidations(CustomerIdentityVerificationResponse customerIdentityVerificationResponse,
	...
	private void activateCustomer(Customer foundCustomer, CustomerKycRequest customerKycRequest,
		...
		String externalId = createOrUpgradeClient(foundCustomer, customerKycRequest, customerKycTier, customer,
				verificationResponse);

		if (StringUtils.isNotBlank(externalId)) {
			AccountDto savingsAccount = createOrRetrieveSavingsAccount(externalId, verificationResponse,
					customerKycTier);
			...
			updateCustomerWithAccountDetails(foundCustomer, externalId, savingsAccount.getAccountNumber(),
					flexAccountNo, savingsAccount.getNuban());
		}
	}

	private String createOrUpgradeClient(Customer foundCustomer, CustomerKycRequest customerKycRequest,
			CustomerKycTier customerKycTier, CustomerDto customer,
			IdentityVerificationResponse.IdentityVerificationResponseData verificationResponse) {

		CreateCustomerRequest createCustomerRequest = buildCreateCustomerRequest(foundCustomer, customerKycRequest,
				customerKycTier, verificationResponse);
	...
	private AccountDto createOrRetrieveSavingsAccount(String externalId,
			IdentityVerificationResponse.IdentityVerificationResponseData verificationResponse,
			CustomerKycTier customerKycTier) {
		...
		if (savingsAccount.isPresent()) {
			...
			PostSavingsAccountsResponse savingsAccountsResponse = accountService.createSavingsAccount(externalId,
					gender, customerKycTier.getCode());
			return clientService.getCustomerWalletAccount(externalId, savingsAccountsResponse.getAccountNumber());
		}
	}

	private String createOrRetrieveFlexInvestmentAccount(String externalId,
			IdentityVerificationResponse.IdentityVerificationResponseData verificationResponse) {
		...
		if (savingsAccount.isPresent()) {
			...
			FlexInvestmentCreationResponse response = flexDepositAccountService
					.submitApplication(flexInvestmentCreationRequest);
	...
	private void updateCustomerWithAccountDetails(Customer foundCustomer, String clientId, String savingsAccountNo,
			String flexAccountNo, String nuban) {
	...
	private void updateCustomerDetails(Customer foundCustomer, CustomerKycRequest customerKycRequest, Long customerId,
		...
		CustomerUpdateRequest customerUpdateRequest = buildUpdateRequest(customerKycRequest, customerKycTier,
				customerIsACorporateUser);
		customerService.updateCustomer(customerUpdateRequest, customerId, foundCustomer, !customerIsACorporateUser);
	}

	private CustomerUpdateRequest buildUpdateRequest(CustomerKycRequest customerKycRequest,
			CustomerKycTier customerKycTier, boolean isCorpoarate) {
	...
	private CreateCustomerRequest buildCreateCustomerRequest(Customer foundCustomer,
			CustomerKycRequest customerKycRequest, CustomerKycTier customerKycTier,
			IdentityVerificationResponse.IdentityVerificationResponseData verificationResponse) {
		...
		setupCommonCustomerFields(createCustomerRequest, foundCustomer, customerKycTier, verificationResponse,
				customerKycRequest);
	...
	private void setupCorporateCustomerRequest(CreateCustomerRequest createCustomerRequest, Customer foundCustomer) {
		...
		IndustrySector industrySector = industrySectorRepository.findById(Long.valueOf(foundCustomer.getIndustryId()))
	...
	private void setupCommonCustomerFields(CreateCustomerRequest createCustomerRequest, Customer foundCustomer,
			CustomerKycTier customerKycTier,
			IdentityVerificationResponse.IdentityVerificationResponseData verificationResponse,
		...
		if (verificationResponse.getAddress() != null) {
			IdentityVerificationResponse.IdentityVerificationResponseData.Address address = verificationResponse
					.getAddress();
	...
	private void updateCustomerAddress(Customer foundCustomer,
	...
	private Address createNewAddress(Long customerId,
			IdentityVerificationResponse.IdentityVerificationResponseData.Address address) {
	...
	private PutClientsClientIdResponse upgradeClientTier(CustomerKycTier customerKycTier, String clientId,
