...
public class CustomerKycServiceImpl implements CustomerKycService {
  ...
  public BaseAppResponse updateCustomerKyc(
  ...
  private BaseAppResponse handleOtpGeneration(
      CustomerKycRequest customerKycRequest, Customer foundCustomer, String command) {

    IdentityVerificationResponse verificationResponse =
        validateKycParameters(customerKycRequest, foundCustomer, command);
    if (verificationResponse != null) {
      return processKycOtpGeneration(
          customerKycRequest, verificationResponse, foundCustomer.getUserType());
    }
    throw new ValidationException(
        "error.generating_otp", "Kyc upgrade failed. Please contact support");
  }

  private GenericApiResponse processKycOtpGeneration(
      CustomerKycRequest customerKycRequest,
      IdentityVerificationResponse verificationResponse,
      UserType userType) {

    NotificationRequestDto notificationRequestDto =
        new NotificationRequestDto(verificationResponse);
    OtpDto otpDto =
        redisService.generateOtpRequest(
            customerKycRequest, OtpType.KYC_UPGRADE, notificationRequestDto, null);
  ...
  private void validateOtpGenerationData(
      IdentityVerificationResponse verificationResponse,
    ...
    if (verificationResponse.getData() != null) {
      ...
      if ("EXTERNAL".equalsIgnoreCase(verificationResponse.getDataSource())) {
        ...
        boolean isValidIndividualData =
            StringUtils.isNoneBlank(data.getFirstName(), data.getLastName(), data.getMobile());

        if ((dataType.isIndividual() && !isValidIndividualData)
            || (dataType.isCorporate() && !isValidCorporateData)) {
          log.error(
              "Data source: {}, Identity verification failed for customer id: {}. Null fields were identified",
              verificationResponse.getDataSource(),
              data);
          throw new ValidationException(
              ...
              "We're unable to complete your tier upgrade because the information provided does not match our records. "
                  ...
                  + dataType
                  + " details are correct or contact your bank customer support for assistance.");
        }
      }
    }
  }
  ...
  private CustomerDtoResponse handleOtpVerification(
      CustomerKycRequest customerKycRequest,
      Long customerId,
      Customer foundCustomer,
      String command) {
    ...
    if (customerKycRequest == null) {
      throw new ValidationException(
          "otp.expired", "Verification has expired or does not exist. Please initiate a new one.");
    }
    ...
    processCustomerActivationOrUpdate(
        foundCustomer, customerKycRequest, customerId, customerKycTier, verificationResponse);
  ...
  private void validateVerificationInput(CustomerKycRequest customerKycRequest) {
    ...
    if (StringUtils.isBlank(customerKycRequest.getBase64Image())
        && StringUtils.isBlank(customerKycRequest.getOtp())) {
      throw new ValidationException(
          "validation.error.exists", "Please provide an otp or a base64Image");
    }
  }

  private OtpDto retrieveAndValidateOtp(
      CustomerKycRequest customerKycRequest, String uniqueId, String otp) {
    ...
    if (StringUtils.isNotBlank(customerKycRequest.getBase64Image())) {
      ...
      identityVerificationService.validateImageMismatch(
          customerKycRequest.getBase64Image(), originalRequest.getBvn(), originalRequest.getNin());
  ...
  private void processCustomerActivationOrUpdate(
      Customer foundCustomer,
      CustomerKycRequest customerKycRequest,
      Long customerId,
    ...
    if (!foundCustomer.isActive()) {
      activateCustomer(
          foundCustomer, customerKycRequest, customerKycTier, verificationResponse.getData());
    } else {
      updateCustomerDetails(foundCustomer, customerKycRequest, customerId, customerKycTier);
    }
  }

  private void updateCustomerAddressIfNeeded(
      Customer foundCustomer, IdentityVerificationResponse verificationResponse) {
  ...
  private IdentityVerificationResponse validateKycParameters(
      CustomerKycRequest customerKycRequest, Customer foundCustomer, String command) {
    ...
    if (StringUtils.isNotBlank(customerKycRequest.getRcNumber())
        && foundCustomer.isCorporateUser()) {
  ...
  private void validateBasicKycRequirements(
      CustomerKycRequest customerKycRequest, Customer foundCustomer) {

    boolean includesNoKycParameter =
        StringUtils.isAllBlank(
            customerKycRequest.getBvn(),
            customerKycRequest.getNin(),
            customerKycRequest.getRcNumber(),
            customerKycRequest.getTin());

    if (!foundCustomer.isActive() && includesNoKycParameter) {
      throw new ValidationException(
          "no.kyc.parameter.provided",
          "At least one KYC parameter (BVN, NIN, TIN or RCNUMBER) must be provided.");
    }

    if (!foundCustomer.isCorporateUser()) {
      if (StringUtils.isBlank(customerKycRequest.getBvn())
          || StringUtils.isBlank(customerKycRequest.getNin())) {
        throw new ValidationException(
            "individual.kyc.parameters.required",
    ...
    if (foundCustomer.isCorporateUser()) {
      if (!foundCustomer.isActive()) {
        if (StringUtils.isBlank(customerKycRequest.getBvn())
            || StringUtils.isBlank(customerKycRequest.getNin())) {
          throw new ValidationException(
              "corporate.customer.not.a.tier1",
  ...
  private IdentityVerificationResponse handleBvnNinValidation(
      CustomerKycRequest customerKycRequest, Customer foundCustomer, String command) {
    ...
    if (GENERATE_OTP_COMMAND.equalsIgnoreCase(command)) {
      IdentityVerificationResponse ninVerificationResponse =
          identityVerificationService.retrieveNinData(customerKycRequest.getNin());
      validateOtpGenerationData(ninVerificationResponse, NIN, foundCustomer);
      IdentityVerificationResponse bvnVerificationResponse =
          identityVerificationService.retrieveBvnData(customerKycRequest.getBvn());
  ...
  private void validateExistingBvnNin(Customer foundCustomer) {
    ...
    if (StringUtils.isNotBlank(foundCustomer.getNin())
        && StringUtils.isNotBlank(foundCustomer.getBvn())) {
      throw new ValidationException(
          "bvn.and.nin.validation.exists", "Both BVN and NIN have already been provided.");
    }
  ...
  private void validateBvnUniqueness(
      CustomerKycRequest customerKycRequest, Customer foundCustomer) {

    log.info("Validating BVN: {}", customerKycRequest.getBvn());
    customerRepository
        .findByBvnAndUserType(customerKycRequest.getBvn(), foundCustomer.getUserType())
        .ifPresent(
            existedCustomer -> {
              log.error(
                  "BVN {} already exists for customer id: {}",
                  customerKycRequest.getBvn(),
                  existedCustomer.getId());
              throw new ValidationException(
                  "bvn.already.exists", "You have entered an invalid BVN");
            });
  }

  private void validateNinUniqueness(
      CustomerKycRequest customerKycRequest, Customer foundCustomer) {

    log.info("Validating NIN: {}", customerKycRequest.getNin());
    customerRepository
        .findByNinAndUserType(customerKycRequest.getNin(), foundCustomer.getUserType())
        .ifPresent(
            existingCustomer -> {
              log.error(
                  "NIN {} already exists for customer id: {}",
                  customerKycRequest.getNin(),
                  existingCustomer.getId());
              throw new ValidationException(
                  "nin.already.exists", "You have entered an invalid NIN.");
            });
  }

  private IdentityVerificationResponse processNinBvnVerification(
      CustomerKycRequest customerKycRequest, Customer foundCustomer) {
    ...
    customerIdentityVerificationResponse =
        identityVerificationService.verifyBvn(customerKycRequest.getBvn(), foundCustomer);
    foundCustomer.setBvn(customerKycRequest.getBvn());
    finalizeValidations(customerIdentityVerificationResponse, BVN);
    IdentityVerificationResponse bvnData =
        identityVerificationService.retrieveBvnData(customerKycRequest.getBvn());
  ...
  private IdentityVerificationResponse handleRcNumberValidation(
      CustomerKycRequest customerKycRequest, Customer foundCustomer, String command) {

    log.info("Validating RC Number: {}", customerKycRequest.getRcNumber());
    BusinessDataResponse businessData =
        identityVerificationService.retrieveRcNumberData(customerKycRequest.getRcNumber());
    IdentityVerificationResponse rcNumberIdentityVerificationResponse =
        IdentityVerificationResponse.parse(businessData.getData(), EXTERNAL);
    if (GENERATE_OTP_COMMAND.equalsIgnoreCase(command)) {
    ...
    finalizeValidations(
        customerIdentityVerificationResponse, IdentityVerificationDataType.RC_NUMBER);
    return rcNumberIdentityVerificationResponse;
  }

  private IdentityVerificationResponse handleTinValidation(
      CustomerKycRequest customerKycRequest, Customer foundCustomer, String command) {

    BusinessDataResponse businessData =
        identityVerificationService.retrieveTinData(customerKycRequest.getTin());
    IdentityVerificationResponse tinIdentityVerificationResponse =
        IdentityVerificationResponse.parse(businessData.getData(), EXTERNAL);
    if (GENERATE_OTP_COMMAND.equalsIgnoreCase(command)) {
  ...
  private static void finalizeValidations(
      CustomerIdentityVerificationResponse customerIdentityVerificationResponse,
      IdentityVerificationDataType dataType) {
  ...
  private void activateCustomer(
      ...
      CustomerKycRequest customerKycRequest,
      CustomerKycTier customerKycTier,
      IdentityVerificationResponse.IdentityVerificationResponseData verificationResponse) {
    ...
    String externalId =
        createOrUpgradeClient(
            foundCustomer, customerKycRequest, customerKycTier, customer, verificationResponse);

    if (StringUtils.isNotBlank(externalId)) {
      AccountDto savingsAccount =
          createOrRetrieveSavingsAccount(externalId, verificationResponse, customerKycTier);
      log.info("Savings account found: {}", savingsAccount);
      String flexAccountNo =
          createOrRetrieveFlexInvestmentAccount(externalId, verificationResponse);
      log.info("Flex account found: {}", flexAccountNo);
      updateCustomerWithAccountDetails(
          foundCustomer,
          externalId,
          savingsAccount.getAccountNumber(),
          flexAccountNo,
          savingsAccount.getNuban());
    }
  }

  private String createOrUpgradeClient(
      Customer foundCustomer,
      CustomerKycRequest customerKycRequest,
      CustomerKycTier customerKycTier,
    ...
    CreateCustomerRequest createCustomerRequest =
        buildCreateCustomerRequest(
            foundCustomer, customerKycRequest, customerKycTier, verificationResponse);
  ...
  private AccountDto createOrRetrieveSavingsAccount(
      String externalId,
      IdentityVerificationResponse.IdentityVerificationResponseData verificationResponse,
      CustomerKycTier customerKycTier) {

    log.info("Client ID found: {}", externalId);
    List<@Valid AccountDto> clientAccounts =
        clientService.getAllWalletAccountByExternalId(externalId);
    ...
    if (savingsAccount.isPresent()) {
      ...
      PostSavingsAccountsResponse savingsAccountsResponse =
          accountService.createSavingsAccount(externalId, gender, customerKycTier.getCode());
      return clientService.getCustomerWalletAccount(
          externalId, savingsAccountsResponse.getAccountNumber());
    }
  }

  private String createOrRetrieveFlexInvestmentAccount(
      String externalId,
      IdentityVerificationResponse.IdentityVerificationResponseData verificationResponse) {

    log.info("Client ID found: {}", externalId);
    List<@Valid AccountDto> clientAccounts =
        clientService.getAllFlexAccountByExternalId(externalId);
    Optional<@Valid AccountDto> savingsAccount = clientAccounts.stream().findAny();
    log.info("Savings account found: {}", savingsAccount);
    ...
    if (savingsAccount.isPresent()) {
      ...
      FlexInvestmentCreationRequest flexInvestmentCreationRequest =
          new FlexInvestmentCreationRequest();
      flexInvestmentCreationRequest.setExternalId(externalId);
      String gender = verificationResponse.getGender();
      flexInvestmentCreationRequest.setGender(gender);
      FlexInvestmentCreationResponse response =
          flexDepositAccountService.submitApplication(flexInvestmentCreationRequest);
      return response != null ? response.getAccountNumber() : null;
    }
  }

  private void updateCustomerWithAccountDetails(
      Customer foundCustomer,
      String clientId,
  ...
  private void updateCustomerDetails(
    ...
    CustomerUpdateRequest customerUpdateRequest =
        buildUpdateRequest(customerKycRequest, customerKycTier, customerIsACorporateUser);
    customerService.updateCustomer(
        customerUpdateRequest, customerId, foundCustomer, !customerIsACorporateUser);
  }

  private CustomerUpdateRequest buildUpdateRequest(
      CustomerKycRequest customerKycRequest,
      CustomerKycTier customerKycTier,
      boolean isCorpoarate) {
  ...
  private CustomerKycTier getCustomerKycTier(
      Customer foundCustomer, CustomerKycRequest customerKycRequest) {
    ...
    if (StringUtils.isAllBlank(foundCustomer.getNin(), foundCustomer.getBvn())) {
      throw new ValidationException(
          "invalid.kyc.tier", "Unable to determine KYC tier for the customer.");
    }

    if (StringUtils.isNoneBlank(foundCustomer.getBvn(), foundCustomer.getNin())) {
      customerKycTier = CustomerKycTier.TIER_2;
    } else if (StringUtils.isNotBlank(foundCustomer.getBvn())
        || StringUtils.isNotBlank(foundCustomer.getNin())) {
    ...
    if (customerKycTier == CustomerKycTier.TIER_2) {
      if (foundCustomer.isCorporateUser()) {
        if (StringUtils.isNotBlank(customerKycRequest.getRcNumber())
            || StringUtils.isNotBlank(customerKycRequest.getTin())) {
  ...
  private void setupCorporateCustomerRequest(
      CreateCustomerRequest createCustomerRequest, Customer foundCustomer) {
    ...
    IndustrySector industrySector =
        industrySectorRepository
            .findById(Long.valueOf(foundCustomer.getIndustryId()))
            .orElse(new IndustrySector());
  ...
  private void setupCommonCustomerFields(
      CreateCustomerRequest createCustomerRequest,
      Customer foundCustomer,
      CustomerKycTier customerKycTier,
      IdentityVerificationResponse.IdentityVerificationResponseData verificationResponse,
    ...
    if (verificationResponse.getAddress() != null) {
      IdentityVerificationResponse.IdentityVerificationResponseData.Address address =
          verificationResponse.getAddress();
  ...
  private PutClientsClientIdResponse upgradeClientTier(
      CustomerKycTier customerKycTier,
      String clientId,
      Customer foundCustomer,
      CustomerKycRequest customerKycRequest) {
  ...
  private GetClientsClientIdResponse fetchCbaClientByEmailOrPhoneNumber(Customer foundCustomer) {
    ...
    client =
        clientService.searchClients(
            null, null, foundCustomer.getEmailAddress(), null, foundCustomer.getUserType());
    if (client == null) {
      client =
          clientService.searchClients(
              null, null, null, foundCustomer.getPhoneNumber(), foundCustomer.getUserType());
    }
    return client;
  }
}
