...
public class CustomerServiceImpl implements CustomerService {
  ...
  @Override
  public BaseAppResponse createCustomer(
    ...
    if ("generate-otp".equalsIgnoreCase(command)) {
      ...
      if (StringUtils.isNotBlank(createCustomerRequest.getTransactionPin())
        ...
        throw new ValidationException(
            "transaction.pin.length", "Transaction pin must be at least 4 characters long.");
      }
      ...
      OtpDto otpDto =
          this.redisService.generateOtpRequest(
              createCustomerRequest, OtpType.ONBOARDING, notificationRequestDto, null);
      ...
      OtpDto otpDto =
          this.redisService.validateOtpWithoutDeletingRecord(
              uniqueId, createCustomerRequest.getOtp(), OtpType.ONBOARDING);
  ...
  private static void validateCreateCustomer(
      CreateCustomerRequest createCustomerRequest, UserType customerType) {
    ...
    if (customerType == UserType.RETAIL) {
      if (StringUtils.isBlank(createCustomerRequest.getEmailAddress())
          && StringUtils.isBlank(createCustomerRequest.getPhoneNumber())) {
        throw new ValidationException(
            "email.or.phone.required",
            "Either email address or phone number is required for retail customers.");
      }
      if (StringUtils.isNotBlank(createCustomerRequest.getBusinessName())) {
        throw new ValidationException(
            "business.name.not.allowed", "Business name is not allowed for retail customers.");
      }
      if (StringUtils.isNotBlank(createCustomerRequest.getRcNumber())) {
        throw new ValidationException(
            "rc.number.not.allowed", "RC number is not allowed for retail customers.");
      }
    } else if (customerType == UserType.CORPORATE) {
      if (StringUtils.isBlank(createCustomerRequest.getBusinessName())) {
        throw new ValidationException(
            "business.name.required", "Business name is required for corporate customers.");
      }
      if (StringUtils.isBlank(createCustomerRequest.getRcNumber())) {
        throw new ValidationException(
            "rc.number.required", "RC number is required for corporate customers.");
      }
    } else {
  ...
  private CustomerDtoResponse completeCustomerRegistration(
    ...
    Customer customer =
        buildCustomer(createCustomerRequest, postClientsResponse, isInitialRegistration);
    Customer savedCustomer = customerRepository.save(customer);
    alertPreferenceService.updatePreference(
        savedCustomer.getId(), AlertType.LOGIN, true, false, false);
    alertPreferenceService.updatePreference(
        savedCustomer.getId(), AlertType.TRANSACTION, true, false, false);
  ...
  public Customer getCustomerById(Long customerId) {
    ...
    return customerRepository
        ...
        .orElseThrow(
            () ->
                new AbstractPlatformResourceNotFoundException(
  ...
  @Override
  public Optional<Customer> getCustomerByEmailAddressAndUserType(
  ...
  @Override
  public Optional<Customer> getCustomerByPhoneNumberAndUserType(
      String phoneNumber, UserType customerType) {
  ...
  public CustomerDashboardResponse retrieveCustomerDashboard(Long customerId) {
    ...
    List<@Valid AccountDto> savingsAccounts =
    ...
    List<@Valid AccountDto> lockAccount =
        lockDepositAccountService.retrieveAllLockInvestmentForAnAccount(
            foundCustomer.getAccountId());
    CustomerDashboardResponse customerDashboardResponse =
        CustomerDashboardResponse.builder()
            .walletAccount(
                new CustomerDashboardResponse.Account(
                    walletAccount.getAccountBalance() == null
                        ? BigDecimal.ZERO
                        : walletAccount.getAccountBalance(),
                    walletAccount.getInterestEarned() == null
                        ? BigDecimal.ZERO
            ...
            .flexAccount(
                new CustomerDashboardResponse.Account(
                    flexAccount.getAccountBalance() == null
                        ? BigDecimal.ZERO
                        : flexAccount.getAccountBalance(),
                    flexAccount.getInterestEarned() == null
                        ? BigDecimal.ZERO
    ...
    customerDashboardResponse.setInvestmentMetrics(
        new CustomerDashboardResponse.Account(
            AppUtil.safeAdd(
                customerDashboardResponse.getFlexAccount().getBalance(),
                customerDashboardResponse.getLockAccount().getBalance()),
            AppUtil.safeAdd(
                customerDashboardResponse.getFlexAccount().getTotalInterestEarned(),
                customerDashboardResponse.getLockAccount().getTotalInterestEarned()),
            AppUtil.safeAdd(
                customerDashboardResponse.getFlexAccount().getTotalDeposit(),
                customerDashboardResponse.getLockAccount().getTotalDeposit()),
            AppUtil.safeAdd(
                customerDashboardResponse.getFlexAccount().getTotalWithdrawal(),
  ...
  @Override
  public GenericApiResponse createTransactionPin(
  ...
  public GenericApiResponse closeAccount(Long customerId, CustomerAccountClosureRequest request) {
    ...
    Optional<CustomerStatusAudit> customerStatusAudit =
        customerStatusAuditRepository.findByCustomerIdAndTypeAndIsActive(
            customer.getId(), CustomerStatusAuditType.ACCOUNT_CLOSURE, true);
    ...
    if (StringUtils.isBlank(request.getReasonForClosure())) {
      throw new ValidationException(
          "validation.error.reasonForClosure", "reasonForClosure is required");
    }
  ...
  @Override
  public Customer getCustomerByEmailOrPhoneNumber(
    ...
    if (StringUtils.isNotBlank(emailAddress)) {
      foundCustomer =
          this.getCustomerByEmailAddressAndUserType(emailAddress, customerType)
              .orElseThrow(
                  () ->
      ...
      foundCustomer =
          this.getCustomerByPhoneNumberAndUserType(phoneNumber, customerType)
              .orElseThrow(
                  () ->
                      new UnAuthenticatedUserException(
                          "Invalid.credentials.provided", "Invalid phone number or password"));
    } else {
      throw new ValidationException(
          "Invalid.credentials.provided", "Email or phone number must be provided");
    }
  ...
  @Override
  public GenericApiResponse activateAccount(
      CustomerAccountActivationRequest request, UserType userType) {

    Customer customer;
    if (request.getCustomerId() == null) {
      if (StringUtils.isAllBlank(request.getEmailAddress(), request.getPhoneNumber())) {
        throw new ValidationException(
            "validation.error.email.or.phone",
            "Either customerId, emailAddress or phoneNumber is required");
      }
      customer =
          this.getCustomerByEmailOrPhoneNumber(
              request.getEmailAddress(), request.getPhoneNumber(), userType);
    } else {
  ...
  private void validateDuplicateCustomerByEmailAddress(String emailAddress, UserType customerType) {
    ...
    getCustomerByEmailAddressAndUserType(emailAddress, customerType)
        .ifPresent(
            existingCustomer -> {
              throw new ValidationException(
                  "customer.exist", "Customer already exists. Please proceed to login");
            });
  }
  ...
  private void validateDuplicateCustomerByPhoneNumber(String phoneNumber, UserType customerType) {
    ...
    log.info("Validating duplicate customer phone number: {}", phoneNumber);
    getCustomerByPhoneNumberAndUserType(phoneNumber, customerType)
        .ifPresent(
            existingCustomer -> {
              throw new ValidationException(
                  "customer.exist", "Customer already exists. Please proceed to login");
  ...
  private void validateDuplicateCustomerByBusinessName(String businessName, UserType customerType) {
    ...
    getCustomerByBusinessName(businessName)
        .ifPresent(
            existingCustomer -> {
              throw new ValidationException(
                  "customer.exist", "Customer already exists. Please proceed to login");
  ...
  public void validateDuplicateCustomerByRcNumber(String rcNumber) {
    ...
    getCustomerByRcNumber(rcNumber)
        .ifPresent(
            existingCustomer -> {
              if (existingCustomer.isActive()) {
                throw new ValidationException(
                    "customer.with.rcNumber.exist",
    ...
    if (customerUpdateRequest != null) {
      if (StringUtils.isNotBlank(customer.getExternalId()) && isExternalServiceUpdateAllowed) {
        clientService.updateCustomer(
            customerUpdateRequest, customer.getExternalId(), customer.getUserType());
      }
  ...
  private void updateCustomerFields(
      CustomerUpdateRequest customerUpdateRequest, Customer customer) {
