...
public class CustomerServiceImpl implements CustomerService {
	...
	public BaseAppResponse createCustomer(CreateCustomerRequest createCustomerRequest, String command,
		...
		if ("generate-otp".equalsIgnoreCase(command)) {
			...
			OtpDto otpDto = this.redisService.validateOtpWithoutDeletingRecord(uniqueId, createCustomerRequest.getOtp(),
					OtpType.ONBOARDING);
	...
	public Customer getCustomerById(Long customerId) {
		...
		return customerRepository.findById(customerId)
				.orElseThrow(() -> new AbstractPlatformResourceNotFoundException("customer.not.found.with.id",
	...
	public GenericApiResponse closeAccount(Long customerId, CustomerAccountClosureRequest request) {
		...
		Optional<CustomerStatusAudit> customerStatusAudit = customerStatusAuditRepository
				.findByCustomerIdAndTypeAndIsActive(customer.getId(), CustomerStatusAuditType.ACCOUNT_CLOSURE, true);
		...
		return GenericApiResponse.builder().status(HttpStatus.OK.name()).message("Account closed successfully").build();
	}
	...
	public GenericApiResponse activateAccount(CustomerAccountActivationRequest request, UserType userType) {
		...
		if (request.getCustomerId() == null) {
			...
			customer = this.getCustomerByEmailOrPhoneNumber(request.getEmailAddress(), request.getPhoneNumber(),
					userType);
		} else {
		...
		return GenericApiResponse.builder().status(HttpStatus.OK.name()).message("Account activated successfully")
				.build();
	}
	...
	public void validateDuplicateCustomerByRcNumber(String rcNumber) {
		...
		getCustomerByRcNumber(rcNumber).ifPresent(existingCustomer -> {
			if (existingCustomer.isActive()) {
				...
				log.error("Customer with rcNumber {} exists but is inactive. Reactivate instead of creating a new one.",
						rcNumber);
			}
		});
	}
	...
	private Customer buildCustomer(CreateCustomerRequest createCustomerRequest, PostClientsResponse postClientsResponse,
