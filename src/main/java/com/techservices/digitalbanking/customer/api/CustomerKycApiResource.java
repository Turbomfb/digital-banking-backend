...
public class CustomerKycApiResource {
	...
	@Operation(
			...
			description = "Updates the KYC (Know Your Customer) information for the authenticated customer. This endpoint allows customers to submit additional documentation and information to upgrade their account tier and increase transaction limits. The system may generate an OTP for verification purposes.",
			requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
					description = "Customer KYC update request containing verification documents and information",
					required = true,
					content = @Content(
	...
	@ApiResponses(value = {
			...
			@ApiResponse(
					responseCode = "409",
					description = "KYC update request already in progress or customer already at maximum tier",
					content = @Content(mediaType = "application/json")
			)
	})
	...
	public ResponseEntity<BaseAppResponse> updateCustomerKyc(
			@Parameter(
					description = "KYC update request containing customer verification documents and information",
					required = true
			)
			@Validated @RequestBody CustomerKycRequest customerKycRequest,

			@Parameter(
					description = "Command to execute during KYC update process. When not specified, OTP generation is skipped.",
	...
	@Operation(
			...
			description = "Retrieves all available KYC tiers in the system along with their requirements, limits, and benefits. This endpoint provides information about different customer verification levels and what each tier allows customers to do within the banking system."
	)
	@ApiResponses(value = {
			@ApiResponse(
					responseCode = "200",
