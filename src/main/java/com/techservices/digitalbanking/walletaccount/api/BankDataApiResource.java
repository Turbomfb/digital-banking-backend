...
@Tag(
		name = "Bank Data Services",
		description = "API for retrieving financial institution data and performing account verification services"
)
...
public class BankDataApiResource {
	...
	@Operation(
			summary = "Retrieve all bank data",
			description = "Fetches comprehensive information about all registered financial institutions including " +
	...
	@ApiResponses(value = {
			@ApiResponse(
					responseCode = "200",
					description = "Successfully retrieved bank data information",
					content = @Content(
							mediaType = "application/json",
							schema = @Schema(implementation = BankDataResponse.class)
					)
			),
			@ApiResponse(
					responseCode = "500",
					description = "Internal server error occurred while retrieving bank data",
					content = @Content(schema = @Schema(hidden = true))
			),
			@ApiResponse(
					responseCode = "503",
					description = "Bank data service temporarily unavailable",
					content = @Content(schema = @Schema(hidden = true))
			)
	})
	@GetMapping
	...
	@Operation(
			summary = "Process account name enquiry",
			description = "Performs real-time account verification by validating account details with the respective " +
	...
	@ApiResponses(value = {
			@ApiResponse(
					responseCode = "200",
					description = "Account name enquiry completed successfully",
					content = @Content(
							mediaType = "application/json",
							schema = @Schema(implementation = NameEnquiryResponse.class)
					)
			),
			@ApiResponse(
					responseCode = "400",
					description = "Invalid request parameters - missing or malformed account number, invalid bank code, or unsupported account type",
					content = @Content(schema = @Schema(hidden = true))
			),
			@ApiResponse(
					responseCode = "404",
					description = "Account not found or bank code not recognized",
					content = @Content(schema = @Schema(hidden = true))
			),
			@ApiResponse(
					responseCode = "422",
					description = "Account verification failed due to inactive account status, restricted account, or validation errors",
					content = @Content(schema = @Schema(hidden = true))
			),
			@ApiResponse(
					responseCode = "500",
					description = "Internal server error occurred during name enquiry processing",
					content = @Content(schema = @Schema(hidden = true))
			),
			@ApiResponse(
					responseCode = "502",
					description = "External bank service error - unable to connect to financial institution's verification service",
					content = @Content(schema = @Schema(hidden = true))
			),
			@ApiResponse(
					responseCode = "504",
					description = "Name enquiry request timeout - financial institution did not respond within acceptable timeframe",
					content = @Content(schema = @Schema(hidden = true))
			)
	})
	...
	public ResponseEntity<NameEnquiryResponse> processNameEnquiry(
			@Parameter(
					description = "Account verification request containing the target account number, " +
							...
							"All fields should be properly formatted and comply with banking standards.",
					required = true,
					content = @Content(
							mediaType = "application/json",
							schema = @Schema(implementation = NameEnquiryRequest.class)
					)
			)
