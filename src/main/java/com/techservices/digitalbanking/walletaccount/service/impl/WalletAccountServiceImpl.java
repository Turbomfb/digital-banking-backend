...
public class WalletAccountServiceImpl implements WalletAccountService {
	...
	public PostSavingsAccountsResponse createSavingsAccount(CreateSavingsAccountRequest createSavingsAccountRequest) {
		...
		return accountService.createSavingsAccount(createSavingsAccountRequest.getClientId(), null, null);
	}
	...
	@Override
	public BasePageResponse<SavingsInterestBreakdownResponse> calculateInterestBreakdown(Long customerId,
		...
		BigDecimal annualInterestRate = BigDecimal
				.valueOf(savingsAccount.getAnnualInterestRate() != null ? savingsAccount.getAnnualInterestRate() : 0)
		...
		while (!currentDate.isAfter(endDate)) {
			...
			BigDecimal dailyInterestRate = annualInterestRate.divide(BigDecimal.valueOf(daysInYear), 8,
					RoundingMode.HALF_UP);

			// Calculate interest for this specific day
			BigDecimal interestForDay = currentBalance.multiply(dailyInterestRate).setScale(4, RoundingMode.HALF_UP);

			currentBalance = currentBalance.add(interestForDay);

			SavingsInterestBreakdownResponse response = SavingsInterestBreakdownResponse.builder().date(currentDate)
					.interestEarned(interestForDay).closingBalance(currentBalance).build();
