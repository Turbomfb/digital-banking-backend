...
public class WalletAccountStatementServiceImpl implements WalletAccountStatementService {
	...
	@Override
	public void generateCsvStatement(StatementRequest request, HttpServletResponse response, Customer customer)
			throws IOException {
		...
		String filename = String.format("statement_%s_%s_%s.csv", accountData.getAccountNumber(),
		...
		try (PrintWriter writer = response.getWriter()) {
			...
			for (TransactionDto transaction : transactions) {
				...
				writer.printf("%s,%s,%s,%s,\"%s\",%s,%s,%s,%s,%s%n", formatDate(LocalDate.from(transaction.getDate())),
						formatDate(LocalDate.from(transaction.getDate())), transaction.getId(),
						StringUtils.defaultString(transaction.getReference(), ""),
						escapeForCsv(getTransactionDescription(transaction)), transactionType,
						formatAmount(debitAmount), formatAmount(creditAmount),
						formatAmount(transaction.getRunningBalance()), "PROCESSED");
			}
	...
	@Override
	public void generatePdfStatement(StatementRequest request, HttpServletResponse response, Customer customer)
			throws IOException {
	...
	@Override
	public void generateExcelStatement(StatementRequest request, HttpServletResponse response, Customer customer)
			throws IOException {
