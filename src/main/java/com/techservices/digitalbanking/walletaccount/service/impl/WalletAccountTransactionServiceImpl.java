...
public class WalletAccountTransactionServiceImpl implements WalletAccountTransactionService {
  ...
  private Executor taskExecutor;
	...
	@Override
	public SavingsAccountTransactionData retrieveSavingsAccountTransactionById(Long customerId, Long transactionId) {
	...
	public BigDecimal getBalanceAsOfDate(Long customerId, LocalDate startDate) {
		...
		return this
				.retrieveSavingsAccountTransactions(customerId, startDate.toString(), null, null, TransactionType.ALL)
				.getData().stream().filter(transaction -> !transaction.getDate().isAfter(startDate.atStartOfDay()))
				.sorted(Comparator.comparing(TransactionDto::getDate)).reduce((first, second) -> second)
				.map(TransactionDto::getRunningBalance).orElse(BigDecimal.ZERO);
	}
	...
	public GenericApiResponse processTransactionCommand(String command, SavingsAccountTransactionRequest request,
		...
		if (GENERATE_OTP_COMMAND.equals(command)) {
			...
			NotificationRequestDto notificationRequestDto = new NotificationRequestDto(sender.getPhoneNumber(),
					sender.getEmailAddress());
			OtpDto otpDto = this.redisService.generateOtpRequest(request, OtpType.TRANSFER, notificationRequestDto,
					request.getAmount());
	...
	public GenericApiResponse receiveInboundWebhook(WalletInboundWebhookRequest request) {
		...
		if (paymentOrder.isPresent()) {
			...
			if (paymentOrderEntity.getAmount().compareTo(request.getAmount()) != 0) {
				log.error("Payment order amount mismatch: expected {}, received {}", paymentOrderEntity.getAmount(),
						request.getAmount());
