...
public class WalletAccountTransactionServiceImpl implements WalletAccountTransactionService {
  ...
  public SavingsAccountTransactionData retrieveSavingsAccountTransactionById(
    ...
    String savingsAccountId = customerService.getCustomerById(customerId).getAccountId();
    return accountTransactionService.retrieveSavingsAccountTransactionById(
  ...
  public BigDecimal getBalanceAsOfDate(Long customerId, LocalDate startDate) {
    ...
    return this.retrieveSavingsAccountTransactions(
            customerId, startDate.toString(), null, null, TransactionType.ALL)
        .getData()
        .stream()
  ...
  @Override
  public GenericApiResponse processTransactionCommand(
      String command, SavingsAccountTransactionRequest request, Long customerId) {
    ...
    if (GENERATE_OTP_COMMAND.equals(command)) {
      ...
      if (!passwordEncoder.matches(request.getTransactionPin(), sender.getTransactionPin())) {
        throw new AbstractPlatformDomainRuleException(
            "error.msg.sender.transaction.pin.mismatch", "Customer transaction pin is not correct");
      }
      request.setTransactionPin(null);
      NotificationRequestDto notificationRequestDto =
          new NotificationRequestDto(sender.getPhoneNumber(), sender.getEmailAddress());
      OtpDto otpDto =
          this.redisService.generateOtpRequest(
              request, OtpType.TRANSFER, notificationRequestDto, request.getAmount());
      String message = notificationRequestDto.getOtpResponseMessage();
      ExternalPaymentTransactionOtpGenerationResponse.Data responseData =
          new ExternalPaymentTransactionOtpGenerationResponse.Data(
      ...
      SavingsAccountTransactionRequest otpData =
          (SavingsAccountTransactionRequest)
              this.redisService
                  .validateOtpWithoutDeletingRecord(
                      request.getReference(), request.getOtp(), OtpType.KYC_UPGRADE)
                  .getData();
      if (otpData.getAmount().compareTo(request.getAmount()) != 0) {
        throw new ValidationException(
            "error.msg.sender.transaction.amount.mismatch", "Invalid payment reference provided");
      }
      ...
      boolean isIntraBankTransfer =
          StringUtils.equalsIgnoreCase(
              request.getBankNipCode(), bankConfigurationService.getBankCode());
      ...
      if (isIntraBankTransfer) {
          ...
          if (recipient.isPresent()) {
            ...
            recipientBalance =
                walletAccountService
                    .retrieveSavingsAccountById(foundRecipient.getId())
                    .getAccountBalance()
                    .add(otpData.getAmount());
            accountTransactionService.processIntraTransafer(
                sender.getAccountId(),
                foundRecipient.getAccountId(),
                request.getAmount(),
                "Transfer | " + request.getNarration());
          } else {
            log.error(
                "Recipient with account number {} not found for intra-bank transfer",
        ...
        try {
          ...
          log.info(
              "Created interbank transaction log with PENDING status: {}",
          ...
          accountTransactionService.handleWithdrawal(
              ...
              "Transfer | " + request.getNarration());
          log.info(
              "Successfully debited sender account for transaction: {}",
              transactionLog.getTransactionReference());
        } catch (Exception e) {
          log.error("Error processing transaction command: {}", e.getMessage(), e);
          throw new ValidationException(
      ...
      try {
        String transactionMessage =
            notificationUtil.getTransactionNotificationTemplate(
    ...
    throw new ValidationException(
        ...
        "Command not supported. Supported commands are: "
            + GENERATE_OTP_COMMAND
            + ", "
            + VERIFY_OTP_COMMAND);
  }

  private TransactionLog createInterbankTransactionLog(
      SavingsAccountTransactionRequest request, Customer customer) {
  ...
  @Override
  public WalletPaymentOrderResponse initiatePaymentOrder(
    ...
    try {
      ...
      log.error(
          "Error initiating payment order for customer {}: {}", customerId, e.getMessage(), e);
      throw new ValidationException(
          "error.payment.order.initiation",
  ...
  public GenericApiResponse receiveInboundWebhook(WalletInboundWebhookRequest request) {
    ...
    Optional<PaymentOrder> paymentOrder =
        paymentOrderRepository.findByReference(request.getReference());
    if (paymentOrder.isPresent()) {
      ...
      if (paymentOrderEntity.getAmount().compareTo(request.getAmount()) != 0) {
        log.error(
            "Payment order amount mismatch: expected {}, received {}",
            paymentOrderEntity.getAmount(),
            request.getAmount());
        throw new ValidationException(
            "error.msg.payment.order.amount.mismatch",
      ...
      if (paymentOrderEntity.getStatus() != PaymentOrderStatus.IN_PROGRESS) {
        ...
        throw new ValidationException(
            "error.msg.payment.order.status.invalid",
      ...
      if (StringUtils.equalsIgnoreCase(PaymentOrderStatus.COMPLETED.name(), request.getStatus())
        ...
        try {
          accountTransactionService.handleDeposit(
              customer.getAccountId(),
              paymentOrderEntity.getAmount(),
              paymentOrderEntity.getReference(),
              "Deposit | " + paymentOrderEntity.getReference());
          ...
          AccountDto accountResponse =
              walletAccountService.retrieveSavingsAccountById(customer.getId());
          BigDecimal balance = accountResponse.getAccountBalance();
          String transactionMessage =
              notificationUtil.getTransactionNotificationTemplate(
                  TransactionType.CREDIT.name(),
                  paymentOrderEntity.getAmount().toString(),
                  balance,
                  paymentOrderEntity.getReference());
          ...
          log.error(
              "Error processing deposit for payment order {}: {}",
              paymentOrderEntity.getReference(),
              e.getMessage());
          throw new ValidationException(
              "error.msg.payment.order.deposit.failed",
              "Failed to process deposit for payment order. Please try again later.");
        }
      }
    }
    throw new ValidationException(
        "error.msg.payment.order.not.found",
        "Payment order not found or invalid. Please check the reference and try again.");
  }

  private Customer validateCustomerAccount(
    ...
    if (!customer.isTransactionPinSet()) {
      ...
      throw new AbstractPlatformDomainRuleException(
          "error.msg.customer.transaction.pin.not.set",
    ...
    if (savingsAccount.getAccountBalance().compareTo(request.getAmount()) < 0) {
      log.error(
          "Insufficient funds: Available balance {} is less than requested amount {}",
