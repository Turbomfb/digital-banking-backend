...
public class InvestmentApiResource {
	...
	public ResponseEntity<InvestmentApplicationResponse> createAnInvestment(
			@Parameter(
					description = "Investment application request details including amount, tenor, and product preferences",
					required = true,
					content = @Content(
	...
	@Operation(
			...
			description = "Updates the details of an existing investment such as auto-renewal settings, liquidation account, or other modifiable parameters. Not all investment parameters can be modified after creation."
	)
	@ApiResponses(value = {
			@ApiResponse(
					responseCode = "200",
			...
			@ApiResponse(
					responseCode = "400",
					description = "Invalid update request or investment cannot be modified in current state",
					content = @Content(mediaType = "application/json")
			),
			@ApiResponse(
					responseCode = "404",
	...
	@Operation(
			...
			description = "Adds funds to a flexible investment account. Flex investments allow for frequent funding and withdrawals with competitive interest rates and high liquidity."
	)
	@ApiResponses(value = {
			@ApiResponse(
					responseCode = "200",
	...
	public ResponseEntity<BaseAppResponse> fundFlexInvestment(
			@Parameter(
					description = "Flex investment funding request with amount and source account details",
					content = @Content(
							schema = @Schema(implementation = InvestmentUpdateRequest.class)
					)
			)
	...
	@Operation(
			...
			description = "Withdraws funds from a flexible investment account. Flex investments offer the flexibility to withdraw funds at any time, though some restrictions may apply based on minimum balance requirements."
	)
	@ApiResponses(value = {
			@ApiResponse(
					responseCode = "200",
	...
	@Operation(
			...
			description = "Calculates potential returns for an investment based on principal amount, interest rate, tenor, and compounding frequency. This tool helps customers make informed investment decisions."
	)
	@ApiResponses(value = {
			@ApiResponse(
					responseCode = "200",
	...
	public ResponseEntity<InvestmentCalculatorResponse> calculateInvestment(
			@Parameter(
					description = "Investment calculation parameters including principal, rate, tenor, and compounding frequency",
					required = true,
					content = @Content(
							schema = @Schema(implementation = InvestmentCalculatorRequest.class)
					)
			)
	...
	@Operation(
			...
			description = "Retrieves all investments belonging to the authenticated customer, filtered by investment type. Returns investments sorted by creation date (newest first) with account details, balances, and status information."
	)
	@ApiResponses(value = {
			@ApiResponse(
					responseCode = "200",
	...
	@Operation(
			...
			description = "Retrieves detailed information about a specific investment including account balance, interest earned, transaction history summary, and investment terms."
	)
	@ApiResponses(value = {
			@ApiResponse(
					responseCode = "200",
	...
	@Operation(
			...
			description = "Retrieves information about the current investment product offerings including interest rates, terms, minimum investment amounts, and product features. This information helps customers understand available investment options."
	)
	@ApiResponses(value = {
			@ApiResponse(
					responseCode = "200",
	...
	public ResponseEntity<Object> retrieveInvestmentTransactionsById(
			...
			@Parameter(
					description = "Filter transactions by staff in selected office only",
					example = "false",
			...
			@Parameter(
					description = "Filter transactions by charge status",
					example = "all",
					schema = @Schema(
							allowableValues = {"all", "paid", "pending", "waived"},
							defaultValue = "all"
					)
			)
