...
public class InvestmentServiceImpl implements InvestmentService {
  ...
  public InvestmentApplicationResponse submitApplication(
    ...
    FixedDepositApplicationRequest fixedDepositApplicationRequest =
        new FixedDepositApplicationRequest();
  ...
  @Override
  public BaseAppResponse updateAnInvestment(
      Long customerId,
    ...
    if (investmentType == InvestmentType.FLEX) {
      PutRecurringDepositProductRequest putRecurringDepositProductRequest =
          new PutRecurringDepositProductRequest();
      if (request.getAmount() != null && request.getAmount().compareTo(BigDecimal.ZERO) > 0) {
        return flexDepositAccountService.updateAnInvestment(
            foundCustomer.getFlexAccountId(), putRecurringDepositProductRequest);
      }
    }
    return null;
  }
  ...
  @Override
  public BaseAppResponse fundInvestment(
      Long customerId,
      InvestmentType investmentType,
      InvestmentUpdateRequest request,
      String investmentId) {
    ...
    if (investmentType == InvestmentType.FLEX) {
      ...
      if (StringUtils.isBlank(foundCustomer.getFlexAccountId())) {
        log.error(
            "Failed to create recurring deposit account for client ID: {}",
      ...
      investmentId =
          StringUtils.isNotBlank(foundCustomer.getFlexAccountId())
      ...
      this.retrieveInvestmentById(investmentId, investmentType, customerId);
      GetFixedDepositAccountsAccountIdResponse response =
          lockDepositAccountService.retrieveInvestmentById(investmentId);
    ...
    return GenericApiResponse.builder()
        .message("Flex account has been funded successfully")
        .build();
  }
  ...
  public BaseAppResponse withdrawFlexInvestment(Long customerId, InvestmentUpdateRequest request) {
    ...
    if (flexAccount.getAccountBalance().compareTo(request.getAmount()) < 0) {
      throw new ValidationException(
          "insufficient.funds", "Insufficient funds in the investment account to withdraw.");
    }
    accountTransactionService.processIntraTransafer(
        flexAccount.getAccountNumber(),
        savingsAccount.getAccountNumber(),
        request.getAmount(),
        "Investment withdrawal");
    return GenericApiResponse.builder()
        .message("Withdrawal from Flex investment successful")
        .build();
  }

  @Override
  public InvestmentCalculatorResponse calculateInvestment(
  ...
  @Override
  public AccountDto retrieveInvestmentById(
      String id, InvestmentType investmentType, Long customerId) {
    ...
    if (InvestmentType.isFlex(investmentType)) {
      ...
      List<@Valid AccountDto> investments =
          lockDepositAccountService.retrieveAllLockInvestmentForAnAccount(
              foundCustomer.getAccountId());
      return investments.stream()
          .filter(
              accountDto -> StringUtils.equals(accountDto.getAccountNumber(), String.valueOf(id)))
          .findFirst()
          .orElseThrow(
              () ->
    ...
    throw new ValidationException(
        "account.not.found", "No investment account found with the provided ID for this customer.");
  }

  @Override
  public BasePageResponse<AccountDto> retrieveAllCustomerInvestments(
  ...
  @Override
  public BasePageResponse<TransactionDto> retrieveInvestmentTransactionsById(
      String id,
    ...
    if ("lock".equalsIgnoreCase(investmentType)) {
      return investmentTransactionService.retrieveAllInvestmentTransactions(
          id, startDate, endDate, size, transactionType);
