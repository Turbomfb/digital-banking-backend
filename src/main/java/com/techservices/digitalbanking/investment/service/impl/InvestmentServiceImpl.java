...
public class InvestmentServiceImpl implements InvestmentService {
	...
	@Override
	public InvestmentApplicationResponse submitApplication(Long customerId, InvestmentApplicationRequest request) {
	...
	@Override
	public BaseAppResponse updateAnInvestment(Long customerId, InvestmentType investmentType,
			InvestmentUpdateRequest request, String investmentId) {
		...
		if (investmentType == InvestmentType.FLEX) {
			...
			if (request.getAmount() != null && request.getAmount().compareTo(BigDecimal.ZERO) > 0) {
				return flexDepositAccountService.updateAnInvestment(foundCustomer.getFlexAccountId(),
						putRecurringDepositProductRequest);
			}
		}
		return null;
	}
	...
	@Override
	public BaseAppResponse fundInvestment(Long customerId, InvestmentType investmentType,
			InvestmentUpdateRequest request, String investmentId) {
		...
		if (investmentType == InvestmentType.FLEX) {
			...
			if (StringUtils.isBlank(foundCustomer.getFlexAccountId())) {
				log.error("Failed to create recurring deposit account for client ID: {}",
						foundCustomer.getExternalId());
				throw new ValidationException("investment.not.found",
						"Error funding investment. Please contact support or try again later.");
			}
			investmentId = StringUtils.isNotBlank(foundCustomer.getFlexAccountId())
					? foundCustomer.getFlexAccountId()
					: investmentId;
			accountTransactionService.processIntraTransafer(savingsAccount.getAccountNumber(), investmentId,
					request.getAmount(), "Investment funding");
		} else if (investmentType == InvestmentType.LOCK) {
			this.retrieveInvestmentById(investmentId, investmentType, customerId);
			GetFixedDepositAccountsAccountIdResponse response = lockDepositAccountService
					.retrieveInvestmentById(investmentId);
	...
	public BaseAppResponse withdrawFlexInvestment(Long customerId, InvestmentUpdateRequest request) {
		...
		accountTransactionService.processIntraTransafer(flexAccount.getAccountNumber(),
				savingsAccount.getAccountNumber(), request.getAmount(), "Investment withdrawal");
	...
	public InvestmentCalculatorResponse calculateInvestment(Long customerId, InvestmentCalculatorRequest request) {
		...
		ProductDto product = lockDepositAccountService.retrieveLockProductDetails();
		...
		BigDecimal interest = amount.multiply(annualInterestRate)
	...
	public AccountDto retrieveInvestmentById(String id, InvestmentType investmentType, Long customerId) {
		...
		if (InvestmentType.isFlex(investmentType)) {
			...
			List<@Valid AccountDto> investments = lockDepositAccountService
					.retrieveAllLockInvestmentForAnAccount(foundCustomer.getAccountId());
			return investments.stream()
					.filter(accountDto -> StringUtils.equals(accountDto.getAccountNumber(), String.valueOf(id)))
					.findFirst().orElseThrow(() -> new ValidationException("account.not.found",
	...
	public BasePageResponse<AccountDto> retrieveAllCustomerInvestments(Long customerId, String investmentType) {
		...
		if (investmentType.equalsIgnoreCase("flex")) {
			...
			return BasePageResponse.instance(response.stream()
					.filter(account -> Objects.equals(account.getId(), foundCustomer.getFlexAccountId())).toList());
		} else {
	...
	@Override
	public BasePageResponse<TransactionDto> retrieveInvestmentTransactionsById(String id, String investmentType,
		...
		if ("lock".equalsIgnoreCase(investmentType)) {
			return investmentTransactionService.retrieveAllInvestmentTransactions(id, startDate, endDate, size,
					transactionType);
