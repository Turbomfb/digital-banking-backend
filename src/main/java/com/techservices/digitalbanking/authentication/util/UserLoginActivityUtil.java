...
public class UserLoginActivityUtil {
	...
	// Known server/cloud provider IP ranges that should not be used for user
	// geolocation
	private static final Set<String> CLOUD_PROVIDER_RANGES = Set.of("34.255.", "34.254.", "34.253.", // AWS Ireland
	...
	public static String extractDeviceNameFromHeaders(HttpServletRequest request, String userAgent) {
		...
		// First priority: Check sec-ch-ua-mobile header (most reliable for mobile
		// detection)
	...
	public static String extractClientIp(HttpServletRequest request) {
		...
		String[] headers = {"CF-Connecting-IP", // Cloudflare (most reliable)
	...
	public static String getLocationFromRequest(String ip, HttpServletRequest request) {
		...
		String[] customLocationHeaders = {"X-User-Location", "X-Geo-Location", "X-Location"};
		...
		// Priority 3: CloudFlare country header (most reliable for country-level
		// location)
		...
		String[] locationHeaders = {"CloudFront-Viewer-Country", "X-Country-Code", "X-Geo-Country", "Geo-Country"};
		...
		if (ip != null && isValidUserIp(ip)) {
			try {
				...
				if (location != null && !location.equalsIgnoreCase("Unknown") && !location.trim().isEmpty()
	...
	private static String mapCountryCodeToName(String countryCode) {
		...
		switch (countryCode.toUpperCase()) {
			...
			case "CO" :
				return "Colombia";
			default :
				return countryCode.toUpperCase(); // Return code if not mapped
		}
	}
	...
	private static String getLocationFromAcceptLanguage(String acceptLanguage) {
		...
		if (lowerLang.contains("en-ng") || lowerLang.contains("yo") || lowerLang.contains("ig")
				|| lowerLang.contains("ha")) {
			return "Nigeria";
