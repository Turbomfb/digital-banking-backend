...
public class UserLoginActivityUtil {
  ...
  // geolocation
  private static final Set<String> CLOUD_PROVIDER_RANGES =
      Set.of(
          "34.255.",
          "34.254.",
          "34.253.", // AWS Ireland
  ...
  public static String getLocationFromRequest(String ip, HttpServletRequest request) {
    ...
    String[] locationHeaders = {
      "CloudFront-Viewer-Country", "X-Country-Code", "X-Geo-Country", "Geo-Country"
    };
    ...
    if (ip != null && isValidUserIp(ip)) {
      try {
        ...
        if (location != null
            ...
            && !location.toLowerCase().contains("dublin")
            && // Filter out server location
  ...
  private static String getLocationFromAcceptLanguage(String acceptLanguage) {
    ...
    if (lowerLang.contains("en-ng")
        || lowerLang.contains("yo")
        || lowerLang.contains("ig")
